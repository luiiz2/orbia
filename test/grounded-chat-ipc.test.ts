...
vi.mock('../src/main/services/chat', () => ({
  GroundedChatService: vi.fn(function () {
    return groundedChat
  }),
...
vi.mock('../src/main/services/retrieval', () => ({
  HybridRetrievalService: vi.fn(function () {
    return {}
  })
}))
vi.mock(
  '../src/main/services/semantic-index/semantic-index-repository.service',
  () => ({
...
describe('Grounded chat and semantic index IPC boundary', () => {
  ...
  it('rejects invalid scope, oversized question and renderer source IDs before invoking services', async () => {
    ...
    await expect(
      Promise.resolve(
        registered.get('chat:resolve-source')!({}, { sourceId: '' })
      )
  ...
  it('registers chat source navigation and semantic queue channels', async () => {
    const { registerChatIpc } = await import('../src/main/ipc/chat.ipc')
    const { registerSemanticIndexIpc } =
      await import('../src/main/ipc/semantic-index.ipc')
    registerChatIpc()
  ...
  it('passes only known grounded chat fields and cancels the matching request', async () => {
    ...
    groundedChat.ask.mockImplementation(
      () =>
        new Promise((resolve) => {
          resolveAsk = resolve
        })
    ...
    await expect(
      Promise.resolve(cancel({}, { requestId: 'request-1' }))
    ).resolves.toBe(true)
  ...
  it('rejects a duplicate active request without replacing the original cancellation controller', async () => {
    ...
    await expect(
      Promise.resolve(cancel({}, { requestId: request.requestId }))
    ).resolves.toBe(true)
  ...
  it('forwards semantic-index worker progress and filters other job types', async () => {
    ...
    registerSemanticIndexIpc()
    const progress = worker.subscribeProgress.mock.calls.at(-1)?.[0] as (
      item: unknown
    ) => void
    const semanticJob = {
      id: 'job-semantic',
  ...
  it('removes exactly the semantic progress listener installed by preload cleanup', async () => {
    ...
    const api = exposedApi as {
      semanticIndex: { onProgress: (callback: () => void) => () => void }
    }
    ...
    expect(ipcRenderer.on).toHaveBeenLastCalledWith(
