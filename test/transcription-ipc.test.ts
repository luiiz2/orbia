...
vi.mock('../src/main/services/transcription/transcription.service', () => ({
...
describe('Transcription IPC boundary', () => {
  beforeEach(async () => {
    ...
    const { registerTranscriptionIpc } =
      await import('../src/main/ipc/transcription.ipc')
  ...
  it('rejects malformed identifiers before reaching the service', async () => {
    ...
    await expect(
      Promise.resolve().then(() => enqueue({}, { lessonId: ' ', options: {} }))
    ).rejects.toThrow('Invalid transcription lesson request')
    expect(service.enqueueLesson).not.toHaveBeenCalled()
  })

  it('sanitizes valid lesson options and forwards the request', async () => {
    const enqueue = handlers.get('transcription:enqueue-lesson')!
    service.enqueueLesson.mockReturnValue({
      lessonId: 'lesson-1',
      skipped: false,
      jobId: 'job-1'
    })
