...
describe('TranscriptionService', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'orbia-transcription-service-')
    )
    ...
    db.prepare(
      `INSERT INTO lessons (id, module_id, course_id, title, order_index, file_path, file_name, file_extension, media_type, duration, file_size, created_at) VALUES ('lesson-1', 'module-1', 'course-1', 'Lesson', 1, ?, 'lesson.mp4', '.mp4', 'video', 10, 5, 1)`
    ).run(mediaPath)
  ...
  it('enqueues lesson, module, and course requests on demand with active-job deduplication', () => {
    expect(
      service.enqueueLesson('lesson-1', { language: 'pt', autoDetect: false })
        .skipped
    ).toBe(false)
    expect(
      service.enqueueLesson('lesson-1', { language: 'pt', autoDetect: false })
    ).toMatchObject({ skipped: true, reason: 'active_job' })
  ...
  it('reuses a suitable subtitle and never calls the provider', async () => {
    ...
    database
      .getDatabase()!
      .prepare(
        `INSERT INTO content_resources (id, course_id, module_id, lesson_id, role, name, file_path, file_extension, file_size, resource_type, language, label, created_at) VALUES ('sub-1', 'course-1', 'module-1', 'lesson-1', 'subtitle', 'lesson.pt.srt', ?, '.srt', 10, 'subtitle', 'pt', 'Português', 1)`
      )
      .run(subtitlePath)
  ...
  it('does not replace the current transcript when retranscription fails', async () => {
    repository.saveCompleted({
      ...
      sourceRevision: 'old',
      settings: {},
      segments: [{ sequence: 0, start: 0, end: 1, text: 'Atual' }]
    })
    engine.transcribe.mockRejectedValueOnce(new Error('provider unavailable'))
    service.enqueueLesson('lesson-1', {
      retranscribe: true,
  ...
  it('preserves the canonical lesson ID when the media path changes', () => {
    ...
    database
      .getDatabase()!
      .prepare(`UPDATE lessons SET file_path = ? WHERE id = 'lesson-1'`)
      .run(movedPath)
    const result = service.enqueueLesson('lesson-1')
