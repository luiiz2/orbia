...
describe('hybrid retrieval', () => {
  ...
  it('fuses lexical and semantic candidates while keeping a lesson scope', async () => {
    await seedCompletedIndex(
      ...
      [
        ...
        chunk(
          ...
          'module-1',
          'lesson-2',
          'Dependency injection from another lesson must stay out.',
          0,
          8
        )
      ],
    ...
    expect(
      result.sources.every((source) => source.lessonId === 'lesson-1')
    ).toBe(true)
    ...
    await seedCompletedIndex(
      ...
      [
        chunk(
          ...
          'module-1',
          'lesson-1',
          'Indexed dependency injection material.',
          0,
          4
        )
      ],
  ...
  it('applies module, course and vault filters in SQL without leaking matching chunks', async () => {
    await seedCompletedIndex(
      ...
      [
        chunk(
          ...
          'module-1',
          'lesson-1',
          'Boundary pattern keeps services isolated.',
          0,
          4
        ),
        chunk(
          ...
          'module-2',
          'lesson-3',
          'Boundary pattern documents another module.',
          0,
          4
        ),
        chunk(
          ...
          'module-3',
          'lesson-4',
          'Boundary pattern belongs to another course.',
          0,
          4
        )
      ],
    ...
    const module = await service.retrieve({
      query: 'boundary pattern',
      scope: { type: 'module', moduleId: 'module-1' }
    })
    const course = await service.retrieve({
      query: 'boundary pattern',
      scope: { type: 'course', courseId: 'course-1' }
    })
    const vault = await service.retrieve({
      query: 'boundary pattern',
      scope: { type: 'vault' }
    })

    expect(module.sources.map((source) => source.moduleId)).toEqual([
      'module-1'
    ])
    expect(
      course.sources.every((source) => source.courseId === 'course-1')
    ).toBe(true)
    expect(course.sources).toHaveLength(2)
    expect(vault.sources.map((source) => source.courseId).sort()).toEqual([
      'course-1',
      'course-1',
      'course-2'
    ])
  ...
  it('keeps lexical sources when the query vector does not match the current generation', async () => {
    ...
    const service = await createService(repository, {
      embed: vi
        .fn()
        .mockResolvedValue(embedding([1, 0, 0], 'openai', 'other-model'))
    })
    ...
    expect(result.sources).toMatchObject([
      { sourceId: 'lexical', lexicalScore: expect.any(Number) }
    ])
  ...
  it('prioritizes the nearest stored timestamp after filtering and caps sources at eight', async () => {
    const chunks = [
      chunk(
        ...
        'module-1',
        'lesson-1',
        'Explain this dependency injection pattern.',
        10,
        20
      ),
      chunk(
        ...
        'module-1',
        'lesson-1',
        'Explain this dependency injection pattern near the current moment.',
        100,
        110
      ),
      ...Array.from({ length: 8 }, (_, index) =>
        chunk(
          ...
          'module-1',
          'lesson-1',
          `Explain this dependency injection pattern extra ${index}.`,
          200 + index,
          201 + index
        )
      )
    ...
    expect(result.sources[0]).toMatchObject({
      sourceId: 'near',
      locator: { startTime: 100, endTime: 110 }
    })
  })
})
...
async function createService(
  ...
  const module =
    (await import('../src/main/services/retrieval/hybrid-retrieval.service')) as unknown as {
...
function seedLibrary(database: DatabaseService, vaultPath: string): void {
  ...
  for (const courseId of ['course-1', 'course-2']) {
    ...
    ).run(courseId, courseId, courseId, 'managed', vaultPath, 1, 1)
  }
  for (const [moduleId, courseId] of [
    ['module-1', 'course-1'],
    ['module-2', 'course-1'],
    ['module-3', 'course-2']
  ] as const) {
    ...
    ).run(moduleId, courseId, moduleId, 1, 10, 1, 1)
  }
  for (const [lessonId, moduleId, courseId] of [
    ['lesson-1', 'module-1', 'course-1'],
    ['lesson-2', 'module-1', 'course-1'],
    ['lesson-3', 'module-2', 'course-1'],
    ['lesson-4', 'module-3', 'course-2']
  ] as const) {
    db.prepare(
      ...
      lessonId,
      1,
      path.join(vaultPath, `${lessonId}.mp4`),
      `${lessonId}.mp4`,
      '.mp4',
      'video',
      10,
...
async function seedCompletedIndex(
  ...
  chunks.forEach((entry, index) =>
    repository.insertSourceChunks(generation.id, [entry], [vectors[index]])
  )
...
function embedding(
  values: number[],
  providerId: AiEmbeddingResponse['providerId'] = 'ollama',
  modelId = 'local-embed'
): AiEmbeddingResponse {
