...
import { TranscriptPanel, highlightTranscriptText } from '../src/renderer/src/components/player/TranscriptPanel'
...
describe('TranscriptPanel', () => {
  it('renders timestamped segments as keyboard-accessible seek controls', () => {
    const markup = renderToStaticMarkup(React.createElement(TranscriptPanel, {
      transcript: {
        id: 'transcript-1',
        lessonId: 'lesson-1',
        version: 1,
  ...
  it('highlights all case-insensitive search matches without changing the source text', () => {
    ...
    expect(parts.filter((part) => part.match).map((part) => part.text)).toEqual(['Olá', 'olá'])
