...
const handlers = new Map<
  string,
  (_event: unknown, payload?: unknown) => Promise<unknown> | unknown
>()
const core = {
  getSettings: vi.fn(),
  saveProvider: vi.fn(),
...
vi.mock('electron', () => ({
  ipcMain: {
    handle: vi.fn(
      (
        channel: string,
        handler: (_event: unknown, payload?: unknown) => unknown
      ) => {
...
vi.mock('../src/main/services/ai/ai-core.service', () => ({
  aiCoreService: core
}))
...
describe('AI IPC boundary', () => {
  ...
  it('rejects invalid provider and chat payloads before reaching the core', async () => {
    ...
    await expect(
      saveProvider(
        {},
        { providerId: 'evil', baseUrl: 'https://evil.test', enabled: true }
      )
    ).rejects.toThrow('Invalid AI provider')
    await expect(
      chat({}, { messages: [{ role: 'user', content: 42 }] })
    ).rejects.toThrow('Invalid AI chat request')
    await expect(
      chat(
        {},
        {
          promptKind: 'unknown',
          messages: [{ role: 'user', content: 'not yet' }]
        }
      )
    ).rejects.toThrow('Invalid AI chat request')
  ...
  it('passes a supported Pull Request prompt kind to the core', async () => {
    ...
    core.chat.mockResolvedValue({
      providerId: 'ollama',
      modelId: 'llama3',
      content: 'TITLE: feat: improve PR prompts'
    })
    ...
    expect(core.chat).toHaveBeenCalledWith(
      expect.objectContaining({ promptKind: 'pull_request' })
    )
  })
  ...
  it('passes valid operations to the core and never adds credential data to snapshots', async () => {
    ...
    expect(core.saveProvider).toHaveBeenCalledWith(
