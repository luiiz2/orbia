...
describe('search IPC bridge', () => {
  ...
  it('validates and routes search:resolve-result request', async () => {
    ...
    expect(librarySearchService.resolveResult).toHaveBeenCalledWith({
      chunkId: 'chunk-123'
    })
  ...
  it('rejects malformed payloads with descriptive validation errors', () => {
    ...
    expect(() => searchHandler({}, 'not-an-object')).toThrow(
      'Invalid library search request'
    )
    expect(() => searchHandler({}, { query: 123 })).toThrow(
      'Invalid library search request'
    )

    const resolveHandler = handlers.get('search:resolve-result')!
    expect(() => resolveHandler({}, null)).toThrow(
      'Invalid search result request'
    )
    expect(() => resolveHandler({}, { chunkId: 123 })).toThrow(
      'Invalid search result request'
    )
  })
})
