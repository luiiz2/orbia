...
function createLegacyContent(
  ...
  `).run(lessonPath, lessonPath, String.raw`E:\Legacy Course\lesson.mp4.bak`, createdAt + 13)
}
...
function removeSourceMigration(db: Database.Database): void {
  ...
  db.prepare(`DELETE FROM _migrations WHERE id = '007_v08_connected_library'`).run()
...
function insertMatchCandidates(db: Database.Database): void {
  ...
  `).run('candidate-lesson', 'lesson-legacy', null, 'legacy-lesson:lesson-legacy')
  db.prepare(`
    INSERT INTO source_match_candidates (
      id, lesson_id, resource_id, source_item_id, confidence, evidence_json,
      review_status, created_at, updated_at
    ) VALUES (?, ?, ?, ?, 0.9, '{}', 'pending', 1, 1)
  `).run('candidate-resource', null, 'resource-legacy', 'legacy-resource:resource-legacy')
}
...
describe('DatabaseService v0.8 connected-library migration', () => {
  ...
  it('creates the fresh source schema, constraints, indexes, and marker without a backup', () => {
    ...
    try {
      ...
      expect(schemaNames).toEqual(expect.arrayContaining([...sourceTables, ...sourceIndexes]))
      expect(
        db.prepare(`SELECT id FROM _migrations WHERE id = '007_v08_connected_library'`).get()
      ).toEqual({ id: '007_v08_connected_library' })
      expect(fs.existsSync(path.join(vault, '.orbia', 'library.db.v0.8.bak'))).toBe(false)
  ...
  it('round-trips every foundational source field and enforces its value constraints', () => {
    ...
    try {
      service.connect(vault)
      const db = getDb(service)
      createLegacyContent(db, 'C:/Library/Lesson.mp4', 'C:/Library', 'C:/Library/Guide.pdf')
      db.prepare(`
      ...
      `).get()).toEqual({ preference_weight: 2.5 })
      expect(db.prepare(`
      ...
      `).get()).toEqual({ is_manual: 1, is_preferred: 1, created_at: 5, updated_at: 6 })
      expect(db.prepare(`
        SELECT decided_at FROM source_match_candidates WHERE id = 'candidate-fields'
      `).get()).toEqual({ decided_at: 7 })
      expect(db.prepare(`
      ...
      expect(() => db.prepare(`UPDATE canonical_source_links SET is_manual = 2 WHERE id = 'link-fields'`).run()).toThrow()
      expect(() => db.prepare(`UPDATE source_items SET width = 0 WHERE id = 'item-fields'`).run()).toThrow()
      expect(() => db.prepare(`UPDATE offline_assets SET availability = 'unknown' WHERE id = 'asset-fields'`).run()).toThrow()
    } finally {
      service.close()
    }
  })
  ...
  it('backs up and losslessly migrates a simulated v0.7 library with deterministic source links', () => {
    ...
    expect(legacyDb.prepare(`SELECT count(*) AS count FROM file_operations`).get()).toEqual({ count: 0 })
    legacyService.close()
    ...
    try {
      ...
      expect(db.prepare(`SELECT count(*) AS count FROM file_operations`).get()).toEqual({ count: 0 })
      expect(db.prepare(`PRAGMA foreign_key_check`).all()).toEqual([])
      expect(db.prepare(`SELECT id, availability FROM content_sources`).all()).toEqual([
        { id: 'legacy-source:course-legacy', availability: 'available' }
      ])
      expect(db.prepare(`SELECT id, source_id, provider_root_identity, local_path, availability FROM source_roots`).all()).toEqual([
        {
      ...
      expect(db.prepare(`SELECT id, locator_json, availability FROM source_items ORDER BY id`).all()).toEqual([
        {
          id: 'legacy-lesson:lesson-legacy',
          locator_json: JSON.stringify({ provider: 'local-folder', path: lessonPath }),
          availability: 'available'
        },
        {
          id: 'legacy-resource:resource-legacy',
          locator_json: JSON.stringify({ provider: 'local-folder', path: resourcePath }),
          availability: 'available'
        }
      ])
      expect(db.prepare(`
      ...
      expect(db.prepare(`SELECT legacy_source_type, legacy_config_json FROM content_sources`).get()).toEqual({
        legacy_source_type: 'external-legacy',
        legacy_config_json: JSON.stringify({ sourceType: 'external-legacy' })
      })
      expect(db.prepare(`SELECT source_type, root_path FROM courses WHERE id = 'course-legacy'`).get()).toEqual({
      ...
      expect(reopenedDb.prepare(`SELECT count(*) AS count FROM source_items`).get()).toEqual({ count: 2 })
      expect(reopenedDb.prepare(`SELECT count(*) AS count FROM canonical_source_links`).get()).toEqual({ count: 2 })

      reopenedDb.prepare(`DELETE FROM content_sources WHERE id = 'legacy-source:course-legacy'`).run()
      expect(reopenedDb.prepare(`SELECT id FROM lessons WHERE id = 'lesson-legacy'`).get()).toEqual({ id: 'lesson-legacy' })
      expect(reopenedDb.prepare(`SELECT id FROM content_resources WHERE id = 'resource-legacy'`).get()).toEqual({ id: 'resource-legacy' })
    } finally {
      migratedService.close()
    }
  })
  ...
  it('marks an empty legacy locator as relink-required without deleting canonical content', () => {
    ...
    try {
      migratedService.connect(vault)
      const db = getDb(migratedService)
      expect(db.prepare(`SELECT locator_json, availability FROM source_items WHERE id = 'legacy-lesson:lesson-legacy'`).get()).toEqual({
        locator_json: JSON.stringify({ provider: 'local-folder', path: '' }),
        availability: 'relink-required'
      })
      expect(db.prepare(`SELECT id, file_path FROM lessons WHERE id = 'lesson-legacy'`).get()).toEqual({
        id: 'lesson-legacy',
        file_path: ''
      })
      expect(db.prepare(`PRAGMA foreign_key_check`).all()).toEqual([])
  ...
  it('marks non-empty unreachable legacy paths as missing without changing canonical paths', () => {
    ...
    try {
      service.connect(vault)
      const db = getDb(service)
      expect(db.prepare(`SELECT availability FROM content_sources`).get()).toEqual({ availability: 'missing' })
      expect(db.prepare(`SELECT local_path, availability FROM source_roots`).get()).toEqual({
        local_path: missingRoot,
        availability: 'missing'
      })
      expect(db.prepare(`SELECT id, availability FROM source_items ORDER BY id`).all()).toEqual([
        { id: 'legacy-lesson:lesson-legacy', availability: 'missing' },
        { id: 'legacy-resource:resource-legacy', availability: 'missing' }
      ])
      expect(db.prepare(`SELECT file_path FROM lessons WHERE id = 'lesson-legacy'`).get()).toEqual({
        file_path: missingLesson
      })
      expect(db.prepare(`SELECT file_path FROM content_resources WHERE id = 'resource-legacy'`).get()).toEqual({
  ...
  it('derives contained Windows and POSIX relative paths with normalized separators', () => {
    ...
    for (const entry of cases) {
      const vault = createVault()
      vaults.push(vault)
      initializeLegacyVault(vault, entry.lessonPath, entry.rootPath, entry.resourcePath)
      const service = new DatabaseService()
      try {
        service.connect(vault)
        expect(getDb(service).prepare(`SELECT id, relative_path FROM source_items ORDER BY id`).all()).toEqual([
          { id: 'legacy-lesson:lesson-legacy', relative_path: entry.expectedLesson },
          { id: 'legacy-resource:resource-legacy', relative_path: entry.expectedResource }
        ])
      } finally {
        service.close()
      }
  ...
  it('falls back to untrusted names when legacy paths are outside the course root', () => {
    ...
    try {
      service.connect(vault)
      const rows = getDb(service).prepare(`SELECT id, relative_path FROM source_items ORDER BY id`).all()
      expect(rows).toEqual([
  ...
  it('allows deleteLesson to cascade source links and candidates without deleting source items', () => {
    ...
    try {
      ...
      expect(db.prepare(`SELECT id FROM lessons WHERE id = 'lesson-legacy'`).get()).toBeUndefined()
      expect(db.prepare(`SELECT count(*) AS count FROM canonical_source_links`).get()).toEqual({ count: 0 })
      expect(db.prepare(`SELECT count(*) AS count FROM source_match_candidates`).get()).toEqual({ count: 0 })
      expect(db.prepare(`SELECT count(*) AS count FROM source_items`).get()).toEqual({ count: 2 })
    } finally {
      service.close()
    }
  })
  ...
  it('allows deleteCourse to cascade source links and candidates without deleting source items', () => {
    ...
    try {
      ...
      expect(db.prepare(`SELECT id FROM courses WHERE id = 'course-legacy'`).get()).toBeUndefined()
      expect(db.prepare(`SELECT count(*) AS count FROM canonical_source_links`).get()).toEqual({ count: 0 })
      expect(db.prepare(`SELECT count(*) AS count FROM source_match_candidates`).get()).toEqual({ count: 0 })
      expect(db.prepare(`SELECT count(*) AS count FROM source_items`).get()).toEqual({ count: 2 })
    } finally {
  ...
  it('backs up a vault containing only Studio user data', () => {
    ...
    try {
      ...
      const backupDb = new Database(backupPath, { readonly: true, fileMustExist: true })
      try {
        expect(backupDb.prepare(`SELECT id, name FROM collections`).get()).toEqual({
          id: 'collection-only',
          name: 'Only user data'
        })
      } finally {
  ...
  it('aborts before backup and migration when the WAL checkpoint is busy', () => {
    ...
    try {
      ...
      expect(reader.prepare(`SELECT id FROM _migrations WHERE id = '007_v08_connected_library'`).get()).toBeUndefined()
    } finally {
      service.close()
      writer.close()
  ...
  it('rejects an existing backup that is not a valid SQLite database', () => {
    ...
    try {
      ...
      try {
        expect(db.prepare(`SELECT id FROM _migrations WHERE id = '007_v08_connected_library'`).get()).toBeUndefined()
      } finally {
        db.close()
      }
    } finally {
  ...
  it('rejects a valid existing backup whose middle row differs', () => {
    ...
    staleBackup.prepare(`UPDATE collections SET name = 'Stale middle' WHERE id = 'collection-middle'`).run()
    staleBackup.close()
  ...
  it('creates a compact openable integrity-clean SQLite snapshot', () => {
    ...
    try {
      ...
      try {
        ...
        expect(backupDb.prepare(`SELECT count(*) AS count FROM collections`).get()).toEqual({ count: 1 })
      } finally {
        backupDb.close()
      }
    } finally {
  ...
  it('removes an unpublished snapshot after failure and retries on the same instance', () => {
    ...
    try {
      try {
        expect(() => service.connect(vault)).toThrow(/injected backup publication failure/i)
      } finally {
      ...
      expect(path.dirname(temporaryBackupPath!)).toBe(path.dirname(backupPath))
      expect(path.basename(temporaryBackupPath!)).toMatch(/^library\.db\.v0\.8\.bak\..+\.tmp$/)
      expect(fs.existsSync(temporaryBackupPath!)).toBe(false)
      expect(fs.existsSync(backupPath)).toBe(false)

      service.connect(vault)
      expect(getDb(service).prepare(`SELECT id FROM _migrations WHERE id = '007_v08_connected_library'`).get()).toEqual({
  ...
  it('cleans a failed connection and reuses its validated backup on same-instance retry', () => {
    ...
    try {
      ...
      expect(repairDb.prepare(`SELECT id FROM _migrations WHERE id = '007_v08_connected_library'`).get()).toBeUndefined()
      repairDb.exec(`DROP TABLE content_sources`)
      repairDb.close()

      service.connect(vault)
      const db = getDb(service)
      expect(db.prepare(`SELECT id FROM _migrations WHERE id = '007_v08_connected_library'`).get()).toEqual({
        id: '007_v08_connected_library'
      })
      expect(snapshotRows(db)).toEqual(before)
