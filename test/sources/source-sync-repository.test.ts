...
describe('SourceRepositoryService source synchronization', () => {
  ...
  it('reserves exact snapshot identities before matching fingerprint renames', () => {
    ...
    repository.completeSync(
      ...
      [
        ...
        },
        {
          ...
          locator: {
            provider: 'local-folder',
            path: 'C:/Course/Module/original.mp4'
          },
          name: 'original.mp4',
    ...
    expect(
      db
        .prepare(
          `SELECT id, availability FROM source_items WHERE provider_item_identity = ?`
        )
        .get('Module/original.mp4')
  ...
  it('does not reuse a fingerprint ID when multiple unmatched snapshot items share it', () => {
      ...
      [
        {
          ...
          locator: {
            provider: 'local-folder',
            path: 'C:/Course/Module/new-a.mp4'
          },
          name: 'new-a.mp4',
        ...
        },
        {
          ...
          locator: {
            provider: 'local-folder',
            path: 'C:/Course/Module/new-b.mp4'
          },
          name: 'new-b.mp4',
    ...
    expect(
      db
        .prepare(
          `SELECT provider_item_identity, availability FROM source_items WHERE id = 'item-linked'`
        )
        .get()
    ).toEqual({
      provider_item_identity: 'Module/original.mp4',
  ...
  it('marks absent items missing without deleting their links', () => {
    ...
    expect(
      db
        .prepare(
          `SELECT availability FROM source_items WHERE id = 'item-linked'`
        )
        .get()
  ...
  it('inserts new snapshot items without canonical links', () => {
    ...
    const item = db
      .prepare(`SELECT id FROM source_items WHERE provider_item_identity = ?`)
      .get('Module/new.mp4') as { id: string }
    expect(item.id).toBeTruthy()
    expect(
      db
        .prepare(
          `SELECT id FROM canonical_source_links WHERE source_item_id = ?`
        )
        .all(item.id)
    ).toEqual([])
    expect(canonicalSnapshot(db)).toEqual(before)
  ...
  it('rejects duplicate provider identities before mutating the snapshot', () => {
        ...
        [
          {
            ...
            locator: {
              provider: 'local-folder',
              path: 'C:/Course/Module/duplicate.mp4'
            },
            name: 'duplicate.mp4',
          ...
          },
          {
            ...
            locator: {
              provider: 'local-folder',
              path: 'C:/Course/Module/duplicate-2.mp4'
            },
    ...
    ).toThrow('Duplicate snapshot provider item identity')
  ...
  it('creates a new item when duplicate fingerprints make a rename ambiguous', () => {
      ...
      [
        {
          ...
          locator: {
            provider: 'local-folder',
            path: 'C:/Course/Module/ambiguous-rename.mp4'
          },
    ...
    expect(
      db
        .prepare(
          `SELECT id, availability FROM source_items WHERE id = 'item-linked'`
        )
        .get()
    ...
    expect(
      db
        .prepare(
          `SELECT id, availability FROM source_items WHERE id = 'item-ambiguous'`
        )
        .get()
    ...
    expect(
      db
        .prepare(`SELECT id FROM source_items WHERE provider_item_identity = ?`)
        .get('Module/ambiguous-rename.mp4')
  ...
  it('marks only the run, root, and source when a sync fails', () => {
    ...
    expect(
      db
        .prepare(
          `SELECT status, error_message, finished_at FROM source_sync_runs WHERE id = ?`
        )
        .get(run.id)
    ).toEqual({
      status: 'failed',
    ...
    expect(
      db
        .prepare(
          `SELECT availability FROM source_roots WHERE id = 'root-local'`
        )
        .get()
    ...
    expect(
      db
        .prepare(
          `SELECT availability FROM content_sources WHERE id = 'source-local'`
        )
        .get()
    ).toEqual({ availability: 'error' })
    expect(db.prepare(`SELECT * FROM source_items ORDER BY id`).all()).toEqual(
      before.items
    )
