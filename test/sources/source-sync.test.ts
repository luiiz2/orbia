...
describe('SourceSyncService', () => {
  ...
  beforeEach(() => {
    vaultPath = fs.mkdtempSync(
      path.join(os.tmpdir(), 'orbia-source-sync-service-')
    )
  ...
  it('accumulates async batches into one snapshot and returns only the sync result', async () => {
    ...
    expect(
      db
        .prepare(
          `SELECT status, scanned_items, changed_items, error_message FROM source_sync_runs`
        )
        .get()
    ).toMatchObject({
      status: 'completed',
  ...
  it('records one sanitized failed run when an adapter errors', async () => {
    ...
    expect(
      db.prepare(`SELECT COUNT(*) AS count FROM source_sync_runs`).get()
    ).toEqual({ count: 1 })
    expect(
      db
        .prepare(
  ...
  it('calls failSync once when completeSync errors after beginSync', async () => {
    ...
    await expect(
      service.syncRoot('root-local', 'manual')
...
function createAdapter(reconcile: SourceAdapter['reconcile']): SourceAdapter {
  return {
    provider: 'local-folder',
    identifyRoot: async () => ({
