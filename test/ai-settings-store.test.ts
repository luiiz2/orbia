...
import { createDefaultAiSettings, type AiModel, type AiProviderHealth, type AiSettingsSnapshot } from '../src/types/ai'
...
describe('AI settings store', () => {
  beforeEach(() => {
    ...
    aiApi.health.mockResolvedValue({ providerId: 'ollama', status: 'AVAILABLE' } as AiProviderHealth)
    ;(globalThis as unknown as { window: { api: { ai: typeof aiApi } } }).window = { api: { ai: aiApi } }
    useAiSettingsStore.setState({ settings: null, models: {}, health: {}, isLoading: false, error: null })
  })
  ...
  it('refreshes model discovery and retains a local unavailable health state', async () => {
    const discovered = [{ id: 'llama3', providerId: 'ollama', capabilities: ['CHAT'] }] as AiModel[]
    aiApi.discoverModels.mockResolvedValueOnce(discovered)
    aiApi.health.mockResolvedValueOnce({ providerId: 'ollama', status: 'CONNECTION_FAILED' } as AiProviderHealth)
    ...
    expect(useAiSettingsStore.getState().health.ollama?.status).toBe('CONNECTION_FAILED')
    expect(useAiSettingsStore.getState().error).toBeNull()
  })

  it('records bridge errors without crashing the settings view', async () => {
    aiApi.discoverModels.mockRejectedValueOnce(new Error('AI provider unavailable'))
