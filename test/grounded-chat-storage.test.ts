...
describe('Phase 4 grounded chat vault migration', () => {
  ...
  it('creates the lexical index and local conversation tables without duplicating source text', () => {
    ...
    expect(db.prepare(`SELECT id FROM _migrations WHERE id = ?`).get('010_v09_grounded_chat')).toBeTruthy()
    expect(db.pragma('journal_mode', { simple: true })).toBe('wal')
    expect(db.pragma('foreign_keys', { simple: true })).toBe(1)

    const sourceColumns = db.prepare(`PRAGMA table_info(chat_message_sources)`).all() as Array<{ name: string }>
    expect(sourceColumns.map((column) => column.name)).not.toContain('text')
    expect(db.prepare(`SELECT sql FROM sqlite_master WHERE name = 'chat_message_sources'`).get()).toMatchObject({
  ...
  it('keeps the FTS index synchronized with Phase 3 chunk inserts, updates and deletes', () => {
    ...
    const search = (term: string) => db.prepare(`
  ...
  it('rebuilds a pre-existing Phase 3 chunk once when the Phase 4 schema is recreated', () => {
    ...
    db.prepare(`
      INSERT INTO courses (id, title, slug, source_type, root_path, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run('course-legacy', 'Legacy course', 'legacy-course', 'managed', vaultPath, 1, 1)
    db.prepare(`
      INSERT INTO semantic_index_generations (id, status, chunking_version, created_at)
      VALUES (?, ?, ?, ?)
    ...
    `)
    db.prepare(`DELETE FROM _migrations WHERE id = ?`).run('010_v09_grounded_chat')
    expect(db.prepare(`SELECT id FROM semantic_index_chunks WHERE id = ?`).get('chunk-legacy')).toBeTruthy()
    ...
    const search = () => firstMigration.prepare(`
      SELECT chunk.id
      FROM semantic_index_fts AS fts
      JOIN semantic_index_chunks AS chunk ON chunk.rowid = fts.rowid
      WHERE fts.semantic_index_fts MATCH ?
    `).all('legacy') as Array<{ id: string }>
    const phaseFourObjects = () => firstMigration.prepare(`
      SELECT type, name
      FROM sqlite_master
      WHERE name IN (
        'semantic_index_fts',
    ...
    expect(firstMigration.prepare(`SELECT COUNT(*) AS count FROM semantic_index_fts`).get()).toEqual({ count: 1 })
    expect(firstMigration.prepare(`SELECT COUNT(*) AS count FROM _migrations WHERE id = ?`).get('010_v09_grounded_chat')).toEqual({ count: 1 })
    ...
    if (!secondMigration) throw new Error('Expected second reconnected database')

    expect(secondMigration.prepare(`
      SELECT chunk.id
      FROM semantic_index_fts AS fts
      JOIN semantic_index_chunks AS chunk ON chunk.rowid = fts.rowid
      WHERE fts.semantic_index_fts MATCH ?
    `).all('legacy')).toEqual([{ id: 'chunk-legacy' }])
    expect(secondMigration.prepare(`SELECT COUNT(*) AS count FROM semantic_index_fts`).get()).toEqual({ count: 1 })
    expect(secondMigration.prepare(`SELECT COUNT(*) AS count FROM _migrations WHERE id = ?`).get('010_v09_grounded_chat')).toEqual({ count: 1 })
    expect(secondMigration.prepare(`
      SELECT type, name
      FROM sqlite_master
      WHERE name IN (
  ...
  it('enforces chat role and status checks while preserving source snapshot provenance', () => {
    ...
    expect(() => db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run('invalid-role', 'conversation-1', 'system', 'Not persisted', now)).toThrow()
    expect(() => db.prepare(`
      INSERT INTO chat_messages (id, conversation_id, role, content, status, created_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run('invalid-status', 'conversation-1', 'assistant', 'Not persisted', 'unknown', now)).toThrow()
  })
})
