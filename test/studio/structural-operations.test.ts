...
describe('Structural Operations Service (Course/Module conversions & Selections)', () => {
  ...
  it('converts a Course into a Module of a target Course preserving lessons', () => {
    ...
    const res = structuralOperationsService.courseToModule(
      db,
      'course_a',
      'course_b'
    )
    ...
    const courseA = db
      .prepare(`SELECT * FROM courses WHERE id = 'course_a'`)
      .get()
    expect(courseA).toBeUndefined()

    // Lessons now belong to course B and new module
    const lessons = db
      .prepare(`SELECT * FROM lessons WHERE course_id = 'course_b'`)
      .all()
  ...
  it('converts a Module into an independent Course', () => {
    ...
    const res = structuralOperationsService.moduleToCourse(
      db,
      'mod_sub',
      'Curso Desmembrado'
    )
    ...
    const newCourse = db
      ...
      .get(res.newCourseId) as { title: string }
    expect(newCourse.title).toBe('Curso Desmembrado')

    const lesson = db
      .prepare(`SELECT course_id FROM lessons WHERE id = 'less_sub1'`)
      .get() as { course_id: string }
  ...
  it('creates a new Course from a selection of appearances', () => {
    ...
    const res = structuralOperationsService.createCourseFromSelection(
      db,
      ['app_l1'],
      'Meu Combo de Aulas'
    )
    ...
    const movedLesson = db
      .prepare(`SELECT course_id FROM lessons WHERE id = 'l1'`)
      .get() as { course_id: string }
