...
describe('Collection Service', () => {
  ...
  it('creates, lists, and adds items to collections', () => {
    ...
    const col = collectionService.createCollection(db, 'Favoritos de Rust', 'Meus materiais prediletos', '#f97316', 'Flame')
    ...
    const addOk = collectionService.addItemsToCollection(db, col.id, ['app_1', 'app_2'])
    expect(addOk).toBe(true)

    const updatedCols = collectionService.listCollections(db)
    expect(updatedCols[0].itemCount).toBe(2)

    const remOk = collectionService.removeItemsFromCollection(db, col.id, ['app_1'])
