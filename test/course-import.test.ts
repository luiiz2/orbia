...
describe('CourseImportService', () => {
  ...
  it('moves staged ZIP content to Courses only after commit and journals the move', async () => {
    ...
    expect(result.course.rootPath).toBe(path.join(fixture.vaultPath, 'Courses', 'curso-seguro-course'))
    expect(fs.existsSync(path.join(result.course.rootPath, 'Module 01', '01 - Lesson.mp4'))).toBe(true)
    expect(fixture.saved).toHaveBeenCalledOnce()
    expect(fixture.recordOperation).toHaveBeenCalledWith(expect.objectContaining({ type: 'move', status: 'pending' }))
    expect(fixture.updateOperation).toHaveBeenCalledWith(expect.any(String), 'completed', null)
    expect(() => fixture.sessions.getSession(fixture.preparedResult.sessionId)).toThrow('Import session not found')
  })

  it('persists an external folder reference when its move manifest is incomplete', async () => {
    const fixture = await createFolderImportFixture(tempRoot, { incompleteMoveManifest: true })
  ...
  it('deletes the original ZIP only after a successful approved commit when the persisted preference is enabled', async () => {
    const fixture = await createZipImportFixture(tempRoot, { deleteSourceZipAfterImport: true })
    ...
    expect(fixture.recordOperation).toHaveBeenCalledWith(expect.objectContaining({ type: 'delete', status: 'pending' }))
    expect(fixture.updateOperation).toHaveBeenCalledWith(expect.any(String), 'completed')
  })
  ...
  it('materializes module materials and lesson subtitles with the final course ownership', async () => {
    ...
    const [, savedModules] = fixture.saved.mock.calls[0] as [
      ...
      Array<{
        id: string
        resources?: Array<{ courseId: string; moduleId: string; filePath: string }>
        lessons: Array<{
          id: string
    ...
    expect(savedLesson.contentResources).toEqual([
      expect.objectContaining({
        ...
        role: 'subtitle',
        filePath: path.join(result.course.rootPath, 'Module 01', '01 - Lesson.pt-BR.vtt')
      })
    ])
  })
  ...
  it('rolls back the staged move and keeps the session when hierarchy persistence fails', async () => {
    ...
    expect(fs.existsSync(path.join(fixture.vaultPath, 'Courses', 'curso-seguro-course'))).toBe(false)
    expect(fs.existsSync(fixture.sourceZip)).toBe(true)
    expect(fixture.updateOperation).toHaveBeenCalledWith(expect.any(String), 'rolled_back', 'database unavailable')
    expect(fixture.sessions.getSession(fixture.preparedResult.sessionId)).toBeDefined()
  })
  ...
  it('keeps the moved course when only import history recording fails', async () => {
    ...
    expect(result.warnings).toContain('Import history could not be recorded: history unavailable')
    expect(fixture.updateOperation).not.toHaveBeenCalledWith(expect.any(String), 'rolled_back', expect.anything())
  })
  ...
  it('keeps the original ZIP and the imported course when delete journaling is unavailable', async () => {
    ...
    expect(fs.existsSync(result.course.rootPath)).toBe(true)
    expect(fs.existsSync(fixture.sourceZip)).toBe(true)
    expect(result.warnings).toContain('The original ZIP was kept because its deletion could not be journaled: delete journal unavailable')
  })
  ...
  it('accepts title edits by IDs while ignoring malicious path payloads', async () => {
    ...
    const [savedCourse, savedModules] = fixture.saved.mock.calls[0] as [
      ...
      Array<{ title: string; folderPath?: string; lessons: Array<{ title: string; filePath: string; coverPath?: string }> }>
    ]
    ...
    expect(savedModules[0].folderPath).toContain(path.join('Courses', 'curso-renomeado-course', 'Module 01'))
    expect(savedModules[0].lessons[0]).toMatchObject({
      title: 'Aula Renomeada',
      coverPath: undefined
    })
    expect(savedModules[0].lessons[0].filePath).toContain(
      path.join('Courses', 'curso-renomeado-course', 'Module 01', '01 - Lesson.mp4')
    )
  ...
  it('does not create persistent covers before the course hierarchy is saved', async () => {
    ...
    const fixture = await createZipImportFixture(tempRoot, { materializeProposal, saved })
  ...
  it('records a pending cover copy before persistCover fails', async () => {
    ...
    const copyFile = vi.spyOn(fs.promises, 'copyFile').mockImplementationOnce(async () => {
      coverWasJournaledBeforeCopy = recordOperation.mock.calls.some(([entry]) => {
        const operation = entry as { type?: string; sourcePath?: string; status?: string }
        return operation.type === 'copy' && operation.sourcePath === generatedCover && operation.status === 'pending'
      })
    ...
    try {
      ...
      const coverJournal = recordOperation.mock.calls
        .map(([entry]) => entry as { operationId: string; type: string; sourcePath: string; destinationPath: string; status: string })
        .find((operation) => operation.type === 'copy' && operation.sourcePath === generatedCover)
      ...
      expect(result.warnings).toContain('Course imported, but covers could not be finalized: cover copy failed')
    } finally {
  ...
  it('cleans an incomplete cross-volume copy without leaving a partial course in Courses', async () => {
    const fixture = await createZipImportFixture(tempRoot)
    const rename = vi.spyOn(fs.promises, 'rename').mockRejectedValueOnce(new Error('cross-device move'))
    const copy = vi.spyOn(fs.promises, 'cp').mockImplementationOnce(async (_source, destination) => {
    ...
    try {
      ...
      expect(fs.existsSync(fixture.extractedPath)).toBe(true)
      expect(fixture.updateOperation).toHaveBeenCalledWith('move-1', 'failed', 'disk full')
    } finally {
      rename.mockRestore()
      copy.mockRestore()
    }
...
async function createZipImportFixture(
  ...
  const moduleResourcePath = path.join(extractedPath, 'Module 01', 'workbook.pdf')
  const lessonSubtitlePath = path.join(extractedPath, 'Module 01', '01 - Lesson.pt-BR.vtt')
  ...
  const proposal: ProposedCourseStructure = {
    ...
    ...(options.proposalCoverPath ? { coverPath: options.proposalCoverPath } : {}),
    modules: [
      {
        id: 'module-1',
        title: 'Dia 1',
  ...
  const archive = {
    ...
    discardPreparedArchive: vi.fn((root: string) => fs.rmSync(root, { recursive: true, force: true }))
  }
  ...
  const preparedResult = await sessions.prepareZipImport({ zipPath: sourceZip, stagingBaseDir })
  ...
  const identifiers = ['course-1', 'group-1', 'move-1', 'module-1', 'lesson-1', 'delete-1']
  const imports = new CourseImportService({
    sessions,
    vault: { getCurrentVault: () => ({ id: 'vault-1', name: 'Vault', path: vaultPath, createdAt: 1, lastOpened: 1 }) },
    database: {
    ...
    settings: { getSettings: () => ({ deleteSourceZipAfterImport: options.deleteSourceZipAfterImport ?? false }) },
    materializeProposal: options.materializeProposal ?? (async (value) => value),
...
async function createFolderImportFixture(
  ...
  const sourceStats = options.incompleteMoveManifest ? await fs.promises.lstat(sourceRoot) : undefined
  ...
  const vaultPath = path.join(tempRoot, 'vault')
  fs.mkdirSync(path.join(vaultPath, 'Courses'), { recursive: true })
  const identifiers = ['course-folder', 'group-folder', 'module-folder', 'lesson-folder']
  const imports = new CourseImportService({
    sessions,
    vault: { getCurrentVault: () => ({ id: 'vault-1', name: 'Vault', path: vaultPath, createdAt: 1, lastOpened: 1 }) },
    database: {
