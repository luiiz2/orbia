...
describe('semantic index chunking', () => {
  ...
  it('normalizes selected scopes and rejects an empty selection', () => {
    ...
    expect(() => normalizeSemanticScope({ type: 'selected' })).toThrow(
      'selected scope'
    )
  })
})
...
describe('semantic index persistence', () => {
  ...
  beforeEach(() => {
    ...
    db.prepare(
      `INSERT INTO transcripts (id, lesson_id, version, language, provider, model, created_at, source_revision, settings_json, status, is_current) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      'transcript-1',
      'lesson-1',
      1,
  ...
  it('stores provenance and Float32 vectors, then publishes only a complete generation', () => {
    ...
    const vectorRow = database
      .getDatabase()!
      .prepare(
        `SELECT vector FROM semantic_index_embeddings WHERE chunk_id = ?`
      )
      .get(repository.listChunks(generation.id)[0].id) as { vector: Buffer }
  ...
  it('replaces only one source and reports partial generations without promoting them', () => {
    ...
    repository.insertSourceChunks(
      generation.id,
      [makeChunk('resource:a', 'A antigo')],
      [[1, 2]]
    )
    repository.insertSourceChunks(
      generation.id,
      [makeChunk('resource:b', 'B')],
      [[3, 4]]
    )
    repository.insertSourceChunks(
      generation.id,
      [makeChunk('resource:a', 'A atualizado')],
      [[5, 6]]
    )

    expect(
      repository.listChunks(generation.id).map((chunk) => chunk.text)
    ).toEqual(expect.arrayContaining(['A atualizado', 'B']))
  ...
  it('keeps notes opt-in and rejects incompatible embedding metadata', () => {
    ...
    expect(() =>
      repository.setEmbeddingConfig(generation.id, 'openai', 'other-model', 3)
  ...
  it('persists semantic jobs in the shared queue without a fake lesson identity', () => {
    ...
    expect(first).toMatchObject({
      ...
      lessonId: '',
      semanticScope: { type: 'vault' }
    })
    expect(duplicate.id).toBe(first.id)
    expect(
      database
        .getDatabase()!
        .prepare(`SELECT lesson_id FROM optimization_queue WHERE id = ?`)
        .get(first.id)
    ).toMatchObject({ lesson_id: null })
...
describe('semantic index content extraction', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'orbia-semantic-extraction-')
    )
    vaultDir = path.join(tempDir, 'vault')
    fs.mkdirSync(vaultDir, { recursive: true })
    ...
    db.prepare(
      `INSERT INTO content_resources (id, course_id, module_id, lesson_id, role, name, file_path, file_extension, file_size, resource_type, language, label, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      'subtitle-1',
      'course-1',
      'module-1',
    ...
    db.prepare(
      `INSERT INTO content_resources (id, course_id, module_id, lesson_id, role, name, file_path, file_extension, file_size, resource_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      'pdf-1',
      'course-1',
      'module-1',
    ...
    ).run('note-1', 'lesson-1', 'course-1', 3, 'Anotação importante', 1, 2)
    ...
    extractor = new ContentExtractorService({
      ...
      pdfExtractor: vi
        .fn()
        .mockResolvedValue([{ page: 1, text: 'Texto extraído do PDF.' }])
    })
  })
  ...
  it('prefers the current transcript over subtitle text and keeps hierarchy metadata', () => {
    const sources = extractor.listSources(
    ...
    expect(
      sources.find((source) => source.sourceId === 'metadata:lesson:lesson-1')
    ).toMatchObject({
      courseId: 'course-1',
      moduleId: 'module-1',
      lessonId: 'lesson-1'
  ...
  it('extracts PDF text and code without duplicating source paths into provenance', async () => {
    ...
    expect(code[0].locator).toMatchObject({
      fileName: 'main.ts',
      language: 'typescript'
    })
  })
  ...
  it('indexes notes only when the vault setting explicitly enables them', () => {
    expect(
      extractor
        .listSources({ type: 'lesson', lessonId: 'lesson-1' }, false)
        .some((source) => source.sourceKind === 'note')
    ).toBe(false)
    expect(
      extractor
        .listSources({ type: 'lesson', lessonId: 'lesson-1' }, true)
        .find((source) => source.sourceKind === 'note')
    ).toMatchObject({
  ...
  it('falls back to a valid subtitle when no current transcript exists and fails unavailable files', async () => {
    ...
    expect(
      sources.find((source) => source.sourceKind === 'subtitle')
    ).toMatchObject({ sourceId: 'lesson:lesson-1:subtitle' })
    const subtitle = await extractor.extractSource(
      sources.find((source) => source.sourceKind === 'subtitle')!
    )
    expect(subtitle[0].segments).toMatchObject([
      { start: 0, end: 1, text: 'Legenda' }
    ])
...
describe('semantic index processing', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'orbia-semantic-processing-')
    )
    vaultDir = path.join(tempDir, 'vault')
    fs.mkdirSync(vaultDir, { recursive: true })
    database = new DatabaseService()
    ...
    for (const resourceId of ['a', 'b']) {
      db.prepare(
        ...
        'resource',
        `${resourceId}.txt`,
        path.join(vaultDir, `${resourceId}.txt`),
        '.txt',
        1,
        'document',
        1
  ...
  function createService(
    ...
    extractSource: ReturnType<typeof vi.fn>
  } {
    const listSources = vi.fn().mockReturnValue(sources)
    const extractSource = vi
      .fn()
      .mockImplementation(async (descriptor: SemanticSourceDescriptor) => {
    ...
    const embed = vi
      .fn()
      .mockImplementation(async (request: { input: string | string[] }) => {
    ...
    const service = new SemanticIndexService({
      ...
      extractor: {
        listSources,
        extractSource
      } as unknown as ContentExtractorService,
  ...
  it('indexes a scope with local embeddings and publishes provenance only after completion', async () => {
    ...
    const { service, embed } = createService(
      [descriptor],
      new Map([
        [descriptor.sourceId, [document(descriptor, 'Texto local indexável.')]]
      ])
    )
  ...
  it('updates only the selected source incrementally and rebuilds from extracted content', async () => {
    ...
    const firstJob = processing.service.enqueue({
      scope: { type: 'lesson', lessonId: 'lesson-1' }
    })
    await processing.service.processJob(firstJob, new AbortController().signal)
    const firstGeneration = repository.getCurrent()!
    const firstB = repository
      .listChunks(firstGeneration.id)
      .find((chunk) => chunk.sourceId === descriptorB.sourceId)!

    const descriptorA2 = source('resource:a', 'revision-a2', 'content-a2')
    documents.set(descriptorA2.sourceId, [
      document(descriptorA2, 'A versão atualizada.')
    ])
    processing.listSources.mockReturnValue([descriptorA2])
    const refreshJob = processing.service.refreshSource(
      { resourceId: 'a' },
      { cloudConsent: true }
    )
    await processing.service.processJob(
      refreshJob,
    ...
    expect(
      afterIncremental.find((chunk) => chunk.sourceId === descriptorA.sourceId)
        ?.text
    ).toBe('A versão atualizada.')
    expect(
      afterIncremental.find((chunk) => chunk.sourceId === descriptorB.sourceId)
        ?.id
    ).toBe(firstB.id)
  ...
  it('keeps a failed source out of the current index and reports a partial generation', async () => {
    ...
    expect(queue.listSemanticIndexQueue()[0]).toMatchObject({
      status: 'partial',
      progressPercent: 100
    })
  })
