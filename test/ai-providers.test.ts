...
describe('AI provider adapters', () => {
  it('discovers Ollama models and maps model capabilities', async () => {
    ...
    const provider = new OllamaProvider(
      providerConfig('ollama', 'http://127.0.0.1:11434'),
      fetchImpl
    )
  ...
  it('calls Ollama chat and embeddings endpoints without requiring a cloud credential', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        response(200, { embeddings: [[0.1, 0.2]], prompt_eval_count: 2 })
      )
    const provider = new OllamaProvider(
      providerConfig('ollama', 'http://127.0.0.1:11434'),
      fetchImpl
    ...
    await expect(
      provider.embed({ modelId: 'nomic-embed-text', input: 'hello' })
    ).resolves.toEqual({
      providerId: 'ollama',
  ...
  it.each([
    ...
    [
      'incomplete segment',
      { text: 'partial transcript', segments: [{ start: 0, end: 1 }] }
    ]
  ])(
    'rejects %s transcription responses instead of returning a false transcript',
    async (_case, body) => {
  ...
  it('does not advertise or invoke transcription for Ollama without a real endpoint', async () => {
    ...
    await expect(
      provider.transcribe({ modelId: 'llama3', audio: new Uint8Array([1]) })
    ).rejects.toMatchObject({
  ...
  it('does not claim embeddings or structured output when discovery omits capabilities', async () => {
    const fetchImpl = vi
      .fn<AiFetch>()
      .mockResolvedValue(response(200, { data: [{ id: 'local-chat' }] }))
    const provider = new OpenAICompatibleProvider(
      providerConfig('openai-compatible', 'http://127.0.0.1:1234/v1'),
      undefined,
      fetchImpl
  ...
  it('normalizes connection failures and invalid credentials', async () => {
    ...
    await expect(localProvider.discoverModels()).rejects.toMatchObject({
      code: 'CONNECTION_FAILED'
    })

    const unauthorized = vi
      .fn<AiFetch>()
      .mockResolvedValue(response(401, { error: 'bad key' }))
    const cloudProvider = new OpenAICompatibleProvider(
      providerConfig('openai', 'https://api.openai.com/v1'),
      'sk-test-secret',
      unauthorized
    )
    await expect(cloudProvider.discoverModels()).rejects.toMatchObject({
      code: 'INVALID_CREDENTIALS'
    })
    await expect(cloudProvider.discoverModels()).rejects.not.toThrow(
      'sk-test-secret'
    )
  })
  ...
  it('bounds a hanging provider request and reports a connection failure', async () => {
    vi.useFakeTimers()
    try {
      const fetchImpl = vi.fn<AiFetch>().mockImplementation(
        (_input, init) =>
