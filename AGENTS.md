...
| **Course Import & Archive Unpack**<br>(Folder scan, zip extract, preview wizard)                        | `src/main/services/scanner.service.ts`<br>`src/main/services/parser.service.ts`<br>`src/main/services/archive.service.ts`<br>`src/main/services/course-import.service.ts` | `src/main/ipc/courses.ipc.ts`<br>(`prepareZipImport`, `scanFolder`, `commitImportSession`) | `stores/useLibraryStore.ts`<br>`stores/useNavigationStore.ts`                                             | `components/import/ImportWizard.tsx`<br>`components/import/ImportModal.tsx`<br>`components/import/ImportPreview.tsx` |
...
│           │   ├── library/       # ContinueWatchingRail, CourseCard
