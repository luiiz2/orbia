...
function createProviderDrafts(
  ...
  return Object.fromEntries(
    AI_PROVIDER_IDS.map((providerId) => {
      ...
      return [
        providerId,
        { baseUrl: provider.baseUrl, enabled: provider.enabled, apiKey: '' }
      ]
    })
...
function requiredCapabilityForTask(
  task: AiTask
): AiModel['capabilities'][number] {
...
function modelOptions(
  ...
  const discovered =
    providerId && models[providerId as AiProviderId]
      ? (models[providerId as AiProviderId] ?? [])
      : []
  const required = requiredCapabilityForTask(task)
  const compatible = discovered.filter((model) =>
    model.capabilities.includes(required)
  )
  const current = currentModel
    ? discovered.find((model) => model.id === currentModel)
    : undefined
...
export function AiSettingsSection(): React.JSX.Element {
  ...
  const [providerDrafts, setProviderDrafts] = useState<Record<
    AiProviderId,
    ProviderDraft
  > | null>(null)
  const [routeDrafts, setRouteDrafts] = useState<Record<
    AiTask,
    RouteDraft
  > | null>(null)
  ...
  const availableProviders = useMemo(
    () =>
      settings
        ? AI_PROVIDER_IDS.filter((providerId) => settings.providers[providerId])
        : [],
    [settings]
  )
    ...
    return (
      <Card>
        <CardContent className="p-5 text-sm text-muted-foreground">
          {t('settings.ai.loading', 'Loading AI settings...')}
        </CardContent>
      </Card>
    )
  }
  ...
  const saveProviderDraft = async (providerId: AiProviderId): Promise<void> => {
    ...
    setProviderDrafts((current) =>
      current
        ? { ...current, [providerId]: { ...current[providerId], apiKey: '' } }
        : current
    )
  }
  ...
  const saveRouteDraft = async (task: AiTask): Promise<void> => {
    ...
    const buildAssignment = (providerId: string, modelId: string) => {
      ...
      const model = models[providerId as AiProviderId]?.find(
    ...
    <Card className="rounded-2xl border border-border/80 bg-card shadow-sm">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-base font-bold">
            {t('settings.ai.title', 'AI')}
          </CardTitle>
        </div>
        <CardDescription className="text-xs">
          {t(
            'settings.ai.description',
            'Configure optional local and cloud AI providers without changing Orbia offline behavior.'
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5 pt-1">
        ...
        <section className="space-y-3">
          <div>
            <p className="text-sm font-semibold text-foreground">
              {t('settings.ai.general', 'General')}
            </p>
            <p className="text-xs text-muted-foreground">
              {t(
                'settings.ai.generalDesc',
                'AI is optional and never runs without an explicit task route.'
              )}
            </p>
          </div>
        </section>
        ...
        <section className="space-y-3 border-t border-border/40 pt-4">
          <div>
            <p className="text-sm font-semibold text-foreground">
              {t('settings.ai.privacySection', 'Privacy')}
            </p>
            <p className="text-xs text-muted-foreground">
              {t(
                'settings.ai.privacyDesc',
                'Cloud routes require explicit configuration and classified data requires explicit consent.'
              )}
            </p>
          </div>
          <label className="flex items-center justify-between gap-3 rounded-xl border border-border/60 bg-secondary/20 p-3 text-sm">
            ...
            <select
              ...
              onChange={(event) =>
                void setPrivacyMode(
                  event.target.value as (typeof AI_PRIVACY_MODES)[number]
                )
              }
              className="rounded-lg border border-border bg-background px-2 py-1.5 text-xs"
              aria-label={t('settings.ai.privacy', 'Privacy mode')}
            >
              {AI_PRIVACY_MODES.map((mode) => (
          ...
          <div className="space-y-2 rounded-xl border border-border/60 bg-secondary/20 p-3">
            <p className="text-xs font-medium">
              {t(
                'settings.ai.cloudDataTypes',
                'Allow these data categories in cloud requests'
              )}
            </p>
              ...
              {AI_DATA_TYPES.map((dataType) => (
                ...
                >
                  <input
                    ...
                    onChange={(event) =>
                      toggleCloudDataType(dataType, event.target.checked)
                    }
                  />
        ...
        <section className="space-y-3 border-t border-border/40 pt-4">
          <div>
            <p className="text-sm font-semibold text-foreground">
              {t('settings.ai.providers', 'Providers')}
            </p>
            <p className="text-xs text-muted-foreground">
              {t(
                'settings.ai.providersDesc',
                'Provider calls stay in the Main process. API keys are stored encrypted and are never displayed.'
              )}
            </p>
          </div>
          {availableProviders.map((providerId) => {
            ...
            return (
              <div
                key={providerId}
                className="space-y-3 rounded-xl border border-border/60 bg-secondary/20 p-3"
              >
                      ...
                      placeholder={
                        provider.apiKeyConfigured
                          ? t(
                              ...
                              'Credential configured — leave blank to keep'
                            )
                          : t('settings.ai.keyPlaceholder', 'Optional API key')
                      }
                      autoComplete="off"
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="text-[11px] text-muted-foreground">
                    {providerHealth?.status ??
                      (provider.apiKeyConfigured
                        ? t(
                            'settings.ai.credentialConfigured',
                            'Credential configured'
                          )
                        : t('settings.ai.notChecked', 'Not checked'))}
                  </span>
                  <div className="flex gap-2">
                    ...
                    >
                      ...
                      {t('settings.ai.refresh', 'Discover models')}
                    </Button>
                    <Button
                      size="xs"
                      onClick={() => void saveProviderDraft(providerId)}
                      disabled={isLoading}
                      className="gap-1.5"
                    >
        ...
        <section className="space-y-3 border-t border-border/40 pt-4">
          <div>
            <p className="text-sm font-semibold text-foreground">
              {t('settings.ai.models', 'Models')}
            </p>
            <p className="text-xs text-muted-foreground">
              {t(
                'settings.ai.modelsDesc',
                'Available models are discovered from each provider when requested.'
              )}
            </p>
          </div>
            ...
            {availableProviders.map((providerId) => (
              ...
              >
                <p className="font-medium">
                  {settings.providers[providerId].displayName}
                </p>
                <p className="mt-1 text-muted-foreground">
                  {(models[providerId] ?? []).length}{' '}
                  {t('settings.ai.modelsFound', 'models found')}
                </p>
              </div>
            ))}
          </div>
        ...
        <section className="space-y-3 border-t border-border/40 pt-4">
          <div>
            <p className="text-sm font-semibold text-foreground">
              {t('settings.ai.routing', 'Task Routing')}
            </p>
            <p className="text-xs text-muted-foreground">
              {t(
                'settings.ai.routingDesc',
                'Choose a primary model and optional fallback for each future AI task.'
              )}
            </p>
          </div>
          {AI_TASKS.map((task) => {
            const draft = routeDrafts[task]
            const primaryModels = modelOptions(
              task,
              draft.primaryProvider,
              models,
              draft.primaryModel
            )
            const fallbackModels = modelOptions(
              task,
            ...
            return (
              <div
                key={task}
                className="grid gap-2 rounded-xl border border-border/60 bg-secondary/20 p-3 md:grid-cols-[auto_1fr_1fr_auto_1fr_1fr] md:items-center"
              >
                ...
                <select
                  ...
                  className="rounded-lg border border-border bg-background px-2 py-1.5 text-xs"
                  aria-label={`${task} primary provider`}
                >
                  <option value="">
                    {t('settings.ai.noProvider', 'No provider')}
                  </option>
                  {availableProviders.map((providerId) => (
                ...
                <select
                  ...
                  className="rounded-lg border border-border bg-background px-2 py-1.5 text-xs"
                  aria-label={`${task} primary model`}
                  disabled={!draft.primaryProvider}
                >
                  <option value="">
                    {t('settings.ai.noModel', 'No model')}
                  </option>
                ...
                <span className="hidden text-center text-[10px] uppercase text-muted-foreground md:block">
                  {t('settings.ai.fallback', 'fallback')}
                </span>
                <select
                  ...
                  className="rounded-lg border border-border bg-background px-2 py-1.5 text-xs"
                  aria-label={`${task} fallback provider`}
                >
                  <option value="">
                    {t('settings.ai.noProvider', 'No provider')}
                  </option>
                ...
                <select
                  ...
                  className="rounded-lg border border-border bg-background px-2 py-1.5 text-xs"
                  aria-label={`${task} fallback model`}
                  disabled={!draft.fallbackProvider}
                >
                  <option value="">
                    {t('settings.ai.noModel', 'No model')}
                  </option>
                ...
                <Button
                  variant="outline"
                  size="xs"
                  onClick={() => void saveRouteDraft(task)}
                  disabled={isLoading}
                  className="gap-1.5 md:col-start-6"
                >
                  <Save className="h-3 w-3" />
                  {t('common.save')}
                </Button>
        ...
        <section className="space-y-3 border-t border-border/40 pt-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-foreground">
                {t('settings.ai.usageTitle', 'Uso Local da IA')}
              </p>
              <p className="text-xs text-muted-foreground">
                {t(
                  'settings.ai.usageSubtitle',
                  'Métricas contabilizadas exclusivamente na máquina local. Zero telemetria.'
                )}
              </p>
            </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    window.confirm(
                      t(
                        'settings.ai.confirmResetUsage',
                        'Deseja zerar os contadores locais de uso?'
                      )
                    )
                  ) {
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
              <p className="text-[11px] text-muted-foreground">
                {t('settings.ai.metricRequests', 'Requisições')}
              </p>
            ...
            <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
              <p className="text-[11px] text-muted-foreground">
                {t('settings.ai.metricTokens', 'Tokens Totais')}
              </p>
              <p className="text-lg font-bold text-foreground">
                {(
            ...
            <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
              <p className="text-[11px] text-muted-foreground">
                {t('settings.ai.metricAudio', 'Áudio Transcrito')}
              </p>
              <p className="text-lg font-bold text-foreground">
                {Math.round((usageStats?.totalTranscriptionSeconds || 0) / 60)}{' '}
                min
              </p>
            </div>
            <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
              <p className="text-[11px] text-muted-foreground">
                {t('settings.ai.metricChunks', 'Chunks Indexados')}
              </p>
              <p className="text-lg font-bold text-foreground">
                {storageStats?.categories.semanticIndex.itemCount ||
                  usageStats?.totalEmbeddedChunks ||
                  0}
              </p>
            </div>
          </div>
        ...
        <section className="space-y-3 border-t border-border/40 pt-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-foreground">
                {t(
                  'settings.ai.storageTitle',
                  'Gerenciamento de Armazenamento de IA'
                )}
              </p>
              <p className="text-xs text-muted-foreground">
                {t(
                  'settings.ai.storageSubtitle',
                  'Visibilidade de dados auxiliares de IA. Arquivos de mídia originais nunca são excluídos.'
                )}
              </p>
            </div>
            <Button
          ...
          <div className="space-y-2">
            {storageStats &&
              Object.values(storageStats.categories).map(
                (cat: AiCategoryStorageStats) => (
                  <div
                    key={cat.category}
                    className="flex items-center justify-between gap-3 rounded-xl border border-border/60 bg-secondary/20 p-3"
                  >
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {cat.itemCount > 0 ? `${cat.itemCount} itens • ` : ''}
                        {(cat.sizeBytes / (1024 * 1024)).toFixed(2)} MB
                      </p>
                    </div>
                    <Button
                      variant="ghost"
                      ...
                      onClick={async () => {
                        if (
                          window.confirm(
                            t(
                              'settings.ai.confirmClearCategory',
                              `Deseja limpar os dados de "${cat.description}"? A mídia original dos cursos permanecerá intacta.`
                            )
                          )
                        ) {
