 ...
 */
export function DocumentLessonView({ className, onBack }: DocumentLessonViewProps): React.JSX.Element {
  const { t } = useTranslation()
  const { activeCourse, activeLesson, toggleComplete, progressMap } = usePlayerStore()
    ...
    if (mediaType === 'link') {
      ...
      if (ext === 'url' || ext === 'webloc') {
        fetch(mediaUrl)
          ...
          .then((content) => {
            const match = content.match(/URL=(https?:\/\/[^\s]+)/i) || content.match(/<string>(https?:\/\/[^<]+)<\/string>/i)
  ...
  const isCompleted = Boolean(activeLesson && progressMap[activeLesson.id]?.completed)
  ...
  return (
    <div className={cn('relative flex h-full w-full flex-col overflow-hidden bg-zinc-950 select-none', className)}>
      {/* Header */}
      ...
      <div className="flex h-full w-full items-center justify-center bg-zinc-950">
        ...
        {textContent !== null && (
          <pre className="h-full w-full overflow-auto whitespace-pre-wrap break-words p-8 text-sm text-zinc-100 font-mono leading-relaxed">
            {loadError ? t('player.documentLoadError') : textContent || t('common.loading')}
          </pre>
        )}

        {(mediaType === 'link' || (mediaType === 'document' && textContent === null) || mediaType === 'archive' || mediaType === 'other') && (
          <div className="flex flex-col items-center justify-center gap-4 p-8 text-center max-w-md">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/10 text-primary border border-primary/20 shadow-md">
              {mediaType === 'link' ? <Link2 className="h-8 w-8" /> : <FileText className="h-8 w-8" />}
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              {mediaType === 'link' && linkUrl && (
                <Button
                  ...
                  className="gap-2 shadow-lg shadow-orange-500/20 bg-gradient-to-r from-orange-500 via-orange-600 to-amber-500 text-primary-foreground font-semibold rounded-xl cursor-pointer"
                >
      ...
      {loadError && (mediaType === 'link' || (mediaType === 'document' && textContent === null)) && (
