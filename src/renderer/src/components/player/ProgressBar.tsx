  ...
  const progressPercent =
    duration > 0
      ? Math.min(
          ...
          Math.max(
            0,
            ((dragTime !== null ? dragTime : currentTime) / duration) * 100
          )
        )
      : 0
  ...
  return (
    ...
    >
      ...
      <div className="relative h-1.5 w-full rounded-full bg-white/20 transition-all duration-150 group-hover:h-2">
        ...
        {/* Played Progress */}
        <div
          className="absolute left-0 top-0 h-full rounded-full bg-primary shadow-sm shadow-primary/50"
        ...
        {bookmarks &&
          duration > 0 &&
          bookmarks.map((bm) => {
            const bmPct = Math.min(
              100,
              Math.max(0, (bm.timestamp / duration) * 100)
            )
            return (
              <div
                key={bm.id}
        ...
        {chapters &&
          duration > 0 &&
          chapters.map((ch) => {
            const chPct = Math.min(
              100,
              Math.max(0, (ch.timestampSeconds / duration) * 100)
            )
        ...
        {/* Scrubber Knob Thumb */}
        <div
          className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white ring-2 ring-primary shadow-lg shadow-primary/50 transition-transform duration-150 h-3.5 w-3.5 group-hover:scale-125 z-30"
