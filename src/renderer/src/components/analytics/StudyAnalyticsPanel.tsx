...
export function StudyAnalyticsPanel({
  ...
  isLoading,
  onGoalChange
}: StudyAnalyticsPanelProps): React.JSX.Element {
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-pulse">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="h-28 rounded-2xl bg-secondary/40 border border-border/50"
          />
        ))}
      </div>
    )
  ...
  const goalProgress = Math.min(
    100,
    Math.round((todayMinutes / Math.max(1, currentDailyGoal)) * 100)
  )
  ...
  const maxDaySeconds = Math.max(
    ...chartDays.map((d) => d.secondsWatched),
    1800
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Streak Card */}
        <div className="relative overflow-hidden rounded-2xl border border-primary/30 bg-card p-4 shadow-sm hover:border-primary/50 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              {t('analytics.streakTitle', 'Ofensiva')}
            </span>
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary/20 text-primary shadow-inner">
          ...
          <div className="mt-2 flex items-baseline gap-1.5">
            ...
            <span className="text-xs font-medium text-muted-foreground">
              {analytics.currentStreakDays === 1
                ? t('analytics.day', 'dia')
                : t('analytics.days', 'dias')}
            </span>
          </div>
          <div className="mt-1 flex items-center gap-1 text-[11px] text-primary/90 font-medium">
            <Sparkles className="h-3 w-3" />
            <span>
              {t('analytics.bestStreak', 'Recorde:')}{' '}
              {analytics.longestStreakDays}{' '}
              {analytics.longestStreakDays === 1
                ? t('analytics.day', 'dia')
                : t('analytics.days', 'dias')}
            </span>
          </div>
        ...
        {/* Total Time Card */}
        <div className="relative overflow-hidden rounded-2xl border border-accent/30 bg-card p-4 shadow-sm hover:border-accent/50 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              {t('analytics.totalTime', 'Tempo Total')}
            </span>
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-accent/20 text-accent shadow-inner">
        ...
        {/* Daily Goal Card */}
        <div className="group relative overflow-hidden rounded-2xl border border-accent/30 bg-card p-4 shadow-sm hover:border-accent/50 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="opacity-80 hover:opacity-100 flex h-7 px-2 items-center gap-1 text-[11px] font-semibold text-accent hover:text-accent rounded-lg bg-accent/15 hover:bg-accent/25 border border-accent/20 transition-all cursor-pointer"
              >
        ...
        <div className="relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-card p-4 shadow-sm hover:border-emerald-500/50 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
          ...
          <div className="mt-2 flex items-baseline gap-1.5">
            ...
            <span className="text-xs font-medium text-muted-foreground">
              {analytics.totalLessonsCompleted === 1
                ? t('analytics.lesson', 'aula')
                : t('analytics.lessons', 'aulas')}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-foreground">
                {t(
                  'analytics.dailyActivityTitle',
                  'Atividade Diária de Estudo (Últimos 14 dias)'
                )}
              </h3>
            </div>
            ...
            <div className="flex h-48 items-end gap-2 pt-6 pb-2 px-1">
              {chartDays.map((day) => {
                ...
                const heightPct = Math.max(
                  8,
                  Math.round((day.secondsWatched / maxDaySeconds) * 100)
                )
                ...
                return (
                  <div
                    key={day.date}
                    className="group relative flex-1 flex flex-col items-center gap-1.5 h-full justify-end"
                  >
                    {/* Tooltip on hover */}
                    <div className="absolute -top-7 opacity-0 group-hover:opacity-100 transition-opacity bg-popover text-popover-foreground text-[10px] font-semibold px-2 py-0.5 rounded shadow border border-border/60 pointer-events-none whitespace-nowrap z-10">
                      {label}: {dayMins} min ({day.lessonsCount}{' '}
                      {day.lessonsCount === 1 ? 'aula' : 'aulas'})
                    </div>
        ...
        <DialogContent className="max-w-md w-[92vw] p-5 rounded-2xl border-border/80 bg-card shadow-2xl space-y-4">
          <DialogHeader className="space-y-1">
            ...
            <DialogDescription className="text-xs text-muted-foreground">
              {t(
                'settings.dailyStudyGoalDesc',
                'Tempo mínimo de estudo por dia para manter sua ofensiva ativa'
              )}
            </DialogDescription>
          </DialogHeader>
