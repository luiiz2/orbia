...
export interface OrbiaApi {
  ...
  courses: {
    ...
    prepareZipImport: (
      input: PrepareImportSourceInput
    ) => Promise<PrepareImportSessionResult>
    prepareFolderImport: (
      input: PrepareImportSourceInput
    ) => Promise<PrepareImportSessionResult>
    cancelImportSession: (
    ...
    ) => Promise<import('./course').ImportHistoryEntry>
    ...
    extractThumbnails: (payload?: { courseId?: string }) => Promise<{
      success: boolean
    ...
    list: () => Promise<Course[]>
    getById: (courseId: string) => Promise<{
      course: Course
    ...
    getCourseHealth: (
      courseId: string
    ) => Promise<import('./course').CourseHealthReport>
    ...
    getReorganizePlan: (courseId: string) => Promise<{
      success: boolean
      plan?: import('./journal').OperationPlan
      error?: string
    }>
    applyReorganizePlan: (
      groupId: string,
      mutations: import('./journal').ProposedFileMutation[],
      courseId: string
    ) => Promise<{ success: boolean; appliedCount?: number; error?: string }>
    undoReorganizePlan: (
      groupId: string
  ...
  player: {
    ...
    getAllProgressSummaries: () => Promise<
    ...
    addWatchHistory: (
      entry: Omit<WatchHistoryEntry, 'id' | 'watchedAt'>
    ) => Promise<void>
    ...
    ) => Promise<import('./progress').StudyAnalytics>
  ...
  transcription: {
    ...
    ) => Promise<TranscriptionEnqueueResult>
    enqueueModule: (
      moduleId: string,
      options?: TranscriptionOptions
    ...
    setCourseAutoTranscribe: (
      courseId: string,
      enabled: boolean
    ) => Promise<boolean>
    onProgress: (
      callback: (event: TranscriptProgressEvent) => void
    ) => () => void
  }
  ...
  bookmarks: {
    create: (bookmark: {
      ...
      title?: string
      color?: string
    }) => Promise<import('./review').VideoBookmark>
    update: (
      id: string,
  ...
  flashcards: {
    create: (card: {
      ...
      question: string
      answer: string
      state?: import('./review').FlashcardState
      dueAt?: number
    }) => Promise<import('./review').Flashcard>
    update: (
      id: string,
      updates: Partial<import('./review').Flashcard>
    ) => Promise<boolean>
    delete: (id: string) => Promise<boolean>
    ...
    review: (
      id: string,
      grade: import('./review').FlashcardReviewGrade
    ) => Promise<{ success: boolean; flashcard?: import('./review').Flashcard }>
  }

  // Study Queue ("Estudar Depois") (v0.3)
  studyQueue: {
    add: (
      entityType: import('./review').StudyQueueEntityType,
      entityId: string
    ) => Promise<import('./review').StudyQueueItem>
  ...
  goals: {
    ...
    }) => Promise<import('./review').CourseGoal>
  ...
  backup: {
    ...
    ) => Promise<import('./review').BackupPreview>
    restore: (backupFilePath: string) => Promise<{
      success: boolean
      restoredCoursesCount: number
      error?: string
    }>
  ...
  sessions: {
    ...
    ) => Promise<import('./review').StudySession>
  ...
  studio: {
    ...
    ) => Promise<import('./studio').LibraryAppearance[]>
    updateAppearance: (
      id: string,
      updates: Partial<import('./studio').LibraryAppearance>
    ) => Promise<boolean>
    createReference: (
      entityType: import('./studio').StudioEntityType,
      entityId: string,
      targetCourseId: string,
      parentAppearanceId?: string
    ) => Promise<import('./studio').LibraryAppearance>
    deleteAppearance: (
      appearanceId: string
    ) => Promise<{ success: boolean; promotedAppearanceId?: string }>
    ...
    createSection: (
      ...
      title: string,
      moduleId?: string
    ) => Promise<import('./studio').LibrarySection>
    updateSection: (
      id: string,
      updates: Partial<import('./studio').LibrarySection>
    ) => Promise<boolean>
    ...
    createCollection: (
      ...
      color?: string,
      icon?: string
    ) => Promise<import('./studio').Collection>
    updateCollection: (
      id: string,
      updates: Partial<import('./studio').Collection>
    ) => Promise<boolean>
    ...
    listCustomFieldDefinitions: () => Promise<
      import('./studio').CustomFieldDefinition[]
    >
    createCustomFieldDefinition: (
      name: string,
      fieldType: import('./studio').CustomFieldType,
      options?: string[]
    ) => Promise<import('./studio').CustomFieldDefinition>
    ...
    ) => Promise<{ success: boolean; newCourseId?: string; error?: string }>
    moveItems: (
      appearanceIds: string[],
      targetParentId: string | null,
      targetCourseId: string
    ) => Promise<{ success: boolean; movedCount: number }>
    createCourseFromSelection: (
      appearanceIds: string[],
      courseTitle: string
    ) => Promise<{ success: boolean; newCourse?: Course; error?: string }>

    // History & Transactional Undo
    listHistory: (
      limit?: number
    ) => Promise<import('./studio').StudioHistoryEntry[]>
    undo: (historyId: string) => Promise<{ success: boolean; error?: string }>

    // Bulk Editing & Spreadsheet Drafts
    renamePreview: (
      appearanceIds: string[],
      options: import('./studio').BulkRenameOptions
    ) => Promise<import('./studio').BulkRenamePreviewItem[]>
    renameApply: (
      items: { appearanceId: string; newTitle: string }[]
    ) => Promise<boolean>
    applySpreadsheetDraft: (
      changes: import('./studio').SpreadsheetDraftChange[]
    ...
    saveAutomationRule: (
      rule: Omit<
        import('./studio').AutomationRule,
        'id' | 'createdAt' | 'updatedAt'
      > & { id?: string }
    ) => Promise<import('./studio').AutomationRule>
    ...
    createProfile: (
      name: string,
      avatarPath?: string
    ) => Promise<import('./theme').LocalProfile>
    updateProfile: (
      id: string,
      updates: Partial<import('./theme').LocalProfile>
    ) => Promise<boolean>
    ...
    saveThemePreset: (
      preset: Omit<import('./theme').ThemePreset, 'id' | 'createdAt'> & {
        id?: string
      }
    ) => Promise<import('./theme').ThemePreset>
    getResolvedTheme: (
      profileId?: string,
      vaultPath?: string,
      courseId?: string,
      sectionId?: string
    ) => Promise<import('./theme').ResolvedTheme>
    saveAppearanceOverride: (
      scopeType: import('./theme').ThemeScope,
      scopeId: string,
      overrides: Partial<import('./theme').ThemeConfig>,
      presetId?: string
    ) => Promise<boolean>
    ...
    importThemePackage: (filePath: string) => Promise<{
      success: boolean
      preset?: import('./theme').ThemePreset
      error?: string
    }>
  }
  ...
  discovery: {
    getDiscoveryRails: (
      profileId?: string
    ) => Promise<import('./discovery').DiscoveryRail[]>
    getSimilarCourses: (
      courseId: string,
      limit?: number
    ) => Promise<import('./discovery').DiscoveryItem[]>
    getTimeBasedRecommendations: (
      minutes: number,
      profileId?: string
    ) => Promise<import('./discovery').TimeBasedRecommendation[]>
    getSurpriseMe: (
      profileId?: string,
      mode?: 'continue' | 'start_new' | 'quick_lesson' | 'random'
    ) => Promise<import('./discovery').SurpriseRecommendation | null>
    getCategoryDiscovery: () => Promise<
      import('./discovery').CategoryDiscoveryData[]
    >
    getLibraryInsights: () => Promise<import('./discovery').LibraryInsights>

    // Profile Discovery Preferences
    getProfileDiscoveryPreferences: (
      profileId: string
    ) => Promise<import('./discovery').ProfileDiscoveryPreferences>
    saveProfileDiscoveryPreferences: (
      preferences: import('./discovery').ProfileDiscoveryPreferences
    ) => Promise<boolean>
    ...
    submitFeedback: (
      profileId: string,
      courseId: string,
      feedbackType: import('./discovery').RecommendationFeedbackType
    ) => Promise<boolean>

    // Course Relationships & Journeys
    listCourseRelationships: (
      courseId?: string
    ) => Promise<import('./discovery').CourseRelationship[]>
    addCourseRelationship: (
      sourceCourseId: string,
      targetCourseId: string,
      relationshipType: import('./discovery').CourseRelationshipType
    ) => Promise<import('./discovery').CourseRelationship>
  ...
  sources: {
    ...
    listCandidates: (
      status?: import('./source').SourceMatchStatus
    ) => Promise<import('./source').SourceMatchCandidateView[]>
    link: (
      sourceItemId: string,
      canonicalType: import('./source').CanonicalSourceType,
      canonicalId: string
    ) => Promise<import('./source').CanonicalSourceLink>
    unlink: (
      sourceItemId: string,
      canonicalType: import('./source').CanonicalSourceType,
      canonicalId: string
    ...
    reviewCandidate: (
      candidateId: string,
      decision: Exclude<import('./source').SourceMatchStatus, 'pending'>
    ) => Promise<import('./source').SourceMatchCandidateView>
    matchRoot: (
      rootId: string
  ...
  optimizer: {
    ...
    ) => Promise<import('./optimizer').VaultOptimizationAnalysis>
    getHardwareCapabilities: () => Promise<
      import('./optimizer').HardwareCapabilities
    >
    ...
    listRecords: (
      limit?: number
    ) => Promise<import('./optimizer').OptimizationRecord[]>
    restoreOriginal: (
      recordId: string
    ) => Promise<{ success: boolean; error?: string }>
    reoptimizeLesson: (
    ...
    updateSettings: (
      settings: Partial<import('./optimizer').OptimizationSettings>
    ) => Promise<boolean>
    listExclusions: () => Promise<
      import('./optimizer').OptimizationExclusionRule[]
    >
    setExclusion: (
    ...
    onProgress: (
      callback: (item: import('./optimizer').OptimizationQueueItem) => void
    ) => () => void
  }
  ...
  ai: {
    ...
    clearStorageCategory: (
      category: import('./ai').AiStorageCategory
    ) => Promise<boolean>
  ...
  semanticIndex: {
    ...
    updateSettings: (
      settings: Partial<SemanticIndexSettings>
    ) => Promise<boolean>
    enqueue: (
      input: SemanticIndexEnqueueInput
  ...
  summaries: {
    get: (
      scope: import('./summaries').SummaryScope
    ) => Promise<import('./summaries').SummaryRecord | null>
    generate: (
      input: import('./summaries').GenerateSummaryRequest
    ) => Promise<import('./summaries').GenerateSummaryResponse>
  ...
  chapters: {
    ...
    generate: (
      input: import('./chapters').GenerateChaptersRequest
    ) => Promise<import('./chapters').GenerateChaptersResponse>
    save: (
      input: import('./chapters').SaveChaptersRequest
    ) => Promise<import('./chapters').LessonChapter[]>
    update: (
      input: import('./chapters').UpdateChapterRequest
    ) => Promise<import('./chapters').LessonChapter>
    delete: (
      input: import('./chapters').DeleteChapterRequest
    ) => Promise<boolean>
  }
  ...
  aiNotes: {
    suggest: (
      input: import('./ai-notes').AiNoteRequest
    ) => Promise<import('./ai-notes').AiNoteSuggestion>
  }
