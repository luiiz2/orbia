...
import type { SemanticChunkLocator, SemanticSourceKind } from '../../../types/semantic-index'
...
export type ChatMessageSourceInput = Omit<ChatMessageSource, 'id' | 'messageId' | 'ordinal'>
...
export class ChatRepository {
  ...
  public constructor(dependencies: ChatRepositoryDependencies | DatabaseService = {}) {
  ...
  public createConversation(title = 'New conversation'): ChatConversation {
    ...
    db.prepare(`INSERT INTO chat_conversations (id, title, created_at, updated_at) VALUES (?, ?, ?, ?)`).run(
      id,
      normalizedTitle,
      now,
      now
    )
    return { id, title: normalizedTitle, messages: [], createdAt: now, updatedAt: now }
  }
  ...
  public getConversation(id: string): ChatConversation | null {
    ...
    const db = this.requireDatabase()
    const row = db.prepare(`SELECT id, title, created_at, updated_at FROM chat_conversations WHERE id = ?`).get(id) as ConversationRow | undefined
    if (!row) return null
    return {
      id: row.id,
      title: row.title,
  ...
  public renameConversation(id: string, title: string): boolean {
    ...
    const normalizedTitle = requireText(title, 'Conversation title')
    const result = this.requireDatabase().prepare(`UPDATE chat_conversations SET title = ?, updated_at = ? WHERE id = ?`).run(
      normalizedTitle,
      this.nextTimestamp(),
      id
    )
  ...
  public deleteConversation(id: string): boolean {
    if (!isIdentifier(id)) return false
    return this.requireDatabase().prepare(`DELETE FROM chat_conversations WHERE id = ?`).run(id).changes > 0
  }

  public appendUserMessage(conversationId: string, input: AppendUserMessageInput): ChatMessage {
    const content = requireText(input?.content, 'Message content')
    if (!input?.scope || typeof input.scope !== 'object') throw new Error('Message scope is required')
  ...
  public appendAssistantMessage(conversationId: string, input: AppendAssistantMessageInput): ChatMessage {
    const content = requireText(input?.content, 'Message content')
    if (!isGroundedChatStatus(input?.status)) throw new Error('Assistant message status is invalid')
    if (!Array.isArray(input.sources)) throw new Error('Assistant message sources are invalid')
  ...
  public getRecentMessages(conversationId: string, maxMessages: number, maxCharacters: number): ChatMessage[] {
    if (!isIdentifier(conversationId) || maxMessages <= 0 || maxCharacters <= 0) return []
    const messageLimit = Math.floor(maxMessages)
    const characterLimit = Math.floor(maxCharacters)
    if (!Number.isFinite(messageLimit) || !Number.isFinite(characterLimit)) return []
  ...
  private insertMessage(
    ...
    if (!isIdentifier(conversationId)) throw new Error('Conversation ID is required')
    const db = this.requireDatabase()
    const conversation = db.prepare(`SELECT id FROM chat_conversations WHERE id = ?`).get(conversationId)
    ...
    db.transaction(() => {
      ...
      input.sources.forEach((source, ordinal) => {
        insertSource.run(
          this.createId(), id, ordinal, source.chunkId, source.sourceKind, source.sourceId,
          source.courseId, source.moduleId ?? null, source.lessonId ?? null, source.resourceId ?? null,
          source.transcriptId ?? null, source.noteId ?? null, source.sourceRevision,
          JSON.stringify(source.locator), source.displayLabel, createdAt
        )
      })
      db.prepare(`UPDATE chat_conversations SET updated_at = ? WHERE id = ?`).run(createdAt, conversationId)
    })()

    return this.listMessages(conversationId).find((message) => message.id === id)!
  }
  ...
  private mapMessage(row: MessageRow): ChatMessage {
    const sources = this.requireDatabase().prepare(`
      SELECT id, message_id, ordinal, chunk_id, source_kind, source_id, course_id,
             module_id, lesson_id, resource_id, transcript_id, note_id, source_revision,
             locator_json, display_label
  ...
  private nextTimestamp(): number {
    ...
    const clock = Number.isFinite(requested) ? Math.floor(requested) : Date.now()
    const row = this.databaseService.getDatabase()?.prepare(`
...
function parseScope(value: string | null): GroundedScope | undefined {
  ...
  try {
    ...
    return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed as GroundedScope : undefined
  } catch {
    return undefined
  }
}
...
function parseLocator(value: string): SemanticChunkLocator {
  try {
    const parsed: unknown = JSON.parse(value)
    return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed as SemanticChunkLocator : {}
  } catch {
    return {}
  }
}
...
function validateSourceInput(source: ChatMessageSourceInput): void {
  if (!source || typeof source !== 'object') throw new Error('Source snapshot is invalid')
  ...
  if (!isSourceKind(source.sourceKind)) throw new Error('Source kind is invalid')
  if (!source.locator || typeof source.locator !== 'object' || Array.isArray(source.locator)) {
...
function isGroundedChatStatus(value: unknown): value is GroundedChatStatus {
  return value === 'answered' || value === 'insufficient_evidence' || value === 'failed' || value === 'cancelled'
}

function isSourceKind(value: unknown): value is SemanticSourceKind {
  return value === 'transcript' || value === 'subtitle' || value === 'pdf' || value === 'markdown' ||
    value === 'text' || value === 'code' || value === 'note' || value === 'metadata'
}

function isDatabaseService(value: ChatRepositoryDependencies | DatabaseService): value is DatabaseService {
