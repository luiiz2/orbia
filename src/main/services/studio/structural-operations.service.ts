...
export class StructuralOperationsService {
  ...
  public courseToModule(
    ...
    if (sourceCourseId === targetCourseId) {
      return { success: false, error: 'O curso de origem não pode ser igual ao de destino' }
    }

    const sourceCourse = db.prepare(`SELECT * FROM courses WHERE id = ?`).get(sourceCourseId) as Course | undefined
    if (!sourceCourse) {
      return { success: false, error: 'Curso de origem não encontrado' }
    }

    const targetCourse = db.prepare(`SELECT * FROM courses WHERE id = ?`).get(targetCourseId) as Course | undefined
    if (!targetCourse) {
    ...
    return db.transaction(() => {
      ...
      `).run({
        ...
        folderPath: (sourceCourse as unknown as Record<string, unknown>).root_path as string || sourceCourse.rootPath || '',
        duration: Number((sourceCourse as unknown as Record<string, unknown>).total_duration ?? sourceCourse.totalDuration ?? 0),
        lessonCount: Number((sourceCourse as unknown as Record<string, unknown>).lesson_count ?? sourceCourse.lessonCount ?? 0),
        createdAt: now
      })
      ...
      const targetCourseApp = db.prepare(`
      ...
      `).run(newModAppId, newModuleId, targetCourseId, targetCourseApp?.id || null, sourceCourse.customTitle || null, displayOrder, now, now)
      ...
      db.prepare(`DELETE FROM library_appearances WHERE entity_id = ? AND entity_type = 'course'`).run(sourceCourseId)
      ...
      const modCount = db.prepare(`SELECT COUNT(id) as modCount FROM modules WHERE course_id = ?`).get(targetCourseId) as { modCount: number }

      db.prepare(`
        UPDATE courses
        SET lesson_count = ?, total_duration = ?, module_count = ?, updated_at = ?
        WHERE id = ?
      `).run(counts.lessonCount, counts.totalDuration, modCount.modCount, now, targetCourseId)
  ...
  public moduleToCourse(
    ...
    const mod = db.prepare(`SELECT * FROM modules WHERE id = ?`).get(moduleId) as Record<string, unknown> | undefined
    if (!mod) {
    ...
    return db.transaction(() => {
      ...
      db.prepare(`UPDATE modules SET course_id = ? WHERE id = ?`).run(newCourseId, moduleId)

      // 3. Update all lessons to new course_id
      db.prepare(`UPDATE lessons SET course_id = ? WHERE module_id = ?`).run(newCourseId, moduleId)

      // 4. Update all content_resources to new course_id
      db.prepare(`UPDATE content_resources SET course_id = ? WHERE module_id = ?`).run(newCourseId, moduleId)
      ...
      `).run(courseAppId, newCourseId, newCourseId, (mod.custom_title as string) || null, now, now)
  ...
  public createCourseFromSelection(
    ...
    return db.transaction(() => {
      const now = Date.now()
      const newCourseId = `course_${crypto.randomUUID()}`
      const slug = `${courseTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}-${now.toString(36)}`
      ...
      const placeholders = appearanceIds.map(() => '?').join(',')
      const selectedApps = db.prepare(`SELECT * FROM library_appearances WHERE id IN (${placeholders})`).all(...appearanceIds) as Array<{
        id: string
        entity_type: string
        entity_id: string
      }>
      ...
      for (const app of selectedApps) {
        if (app.entity_type === 'lesson') {
          ...
          db.prepare(`UPDATE lessons SET course_id = ?, module_id = ? WHERE id = ?`).run(newCourseId, defaultModId, app.entity_id)
          ...
          db.prepare(`UPDATE modules SET course_id = ? WHERE id = ?`).run(newCourseId, app.entity_id)
          db.prepare(`UPDATE lessons SET course_id = ? WHERE module_id = ?`).run(newCourseId, app.entity_id)
        }
      }
      ...
      const modCount = db.prepare(`SELECT COUNT(id) as modCount FROM modules WHERE course_id = ?`).get(newCourseId) as { modCount: number }

      db.prepare(`
        UPDATE courses
        SET lesson_count = ?, total_duration = ?, module_count = ?, updated_at = ?
        WHERE id = ?
      `).run(counts.lessonCount, counts.totalDuration, modCount.modCount, now, newCourseId)

      const newCourse = db.prepare(`SELECT * FROM courses WHERE id = ?`).get(newCourseId) as Course
    ...
    return db.transaction(() => {
      ...
      for (const change of changes) {
        if (change.field === 'customTitle') {
          db.prepare(`UPDATE library_appearances SET custom_title = ?, updated_at = ? WHERE id = ?`).run(change.newValue, now, change.appearanceId)
          appliedCount++
        } else if (change.field === 'displayOrder') {
          db.prepare(`UPDATE library_appearances SET display_order = ?, updated_at = ? WHERE id = ?`).run(change.newValue, now, change.appearanceId)
          appliedCount++
        } else if (change.field === 'sectionId') {
          db.prepare(`UPDATE library_appearances SET section_id = ?, updated_at = ? WHERE id = ?`).run(change.newValue, now, change.appearanceId)
          appliedCount++
        } else if (change.field === 'parentAppearanceId') {
          db.prepare(`UPDATE library_appearances SET parent_appearance_id = ?, updated_at = ? WHERE id = ?`).run(change.newValue, now, change.appearanceId)
          appliedCount++
        } else if (change.field === 'tags') {
          db.prepare(`UPDATE library_appearances SET tags = ?, updated_at = ? WHERE id = ?`).run(JSON.stringify(change.newValue), now, change.appearanceId)
          appliedCount++
        } else if (change.field === 'customMetadata') {
          db.prepare(`UPDATE library_appearances SET custom_metadata = ?, updated_at = ? WHERE id = ?`).run(JSON.stringify(change.newValue), now, change.appearanceId)
          appliedCount++
        } else if (change.field === 'isHidden') {
          db.prepare(`UPDATE library_appearances SET is_hidden = ?, updated_at = ? WHERE id = ?`).run(change.newValue ? 1 : 0, now, change.appearanceId)
          appliedCount++
        }
      }
