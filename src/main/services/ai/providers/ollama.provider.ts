import type { AiCapability, AiModel, AiProviderConfig, AiProviderHealth } from '../../../../types/ai'
import {
  AiProviderError,
  HttpAiProvider,
  normalizeContent,
...
const OLLAMA_CAPABILITIES = ['CHAT', 'EMBEDDINGS', 'STRUCTURED_OUTPUT'] as const satisfies readonly AiCapability[]
...
export class OllamaProvider extends HttpAiProvider {
  ...
  public async discoverModels(): Promise<AiModel[]> {
    const payload = (await this.requestJson('/api/tags')) as { models?: unknown }
  ...
  public async embed(request: AiProviderEmbeddingRequest) {
    ...
    const value = payload && typeof payload === 'object' ? (payload as { embeddings?: unknown; embedding?: unknown }) : {}
    const embeddings = Array.isArray(value.embeddings)
      ? value.embeddings
      : Array.isArray(value.embedding)
        ? [value.embedding]
        : []
    if (!embeddings.every((embedding) => Array.isArray(embedding) && embedding.every((item) => typeof item === 'number'))) {
      throw new AiProviderError('PROVIDER_ERROR', 'AI provider returned invalid embeddings', this.providerId)
    }
    return { providerId: this.providerId, modelId: request.modelId, embeddings: embeddings as number[][] }
  }
}
