...
export class OpenAICompatibleProvider extends HttpAiProvider {
  ...
  public async transcribe(
    ...
    form.append(
      'file',
      new Blob([request.audio as unknown as BlobPart], { type: mimeType }),
      fileName
    )
    form.append('model', request.modelId)
    form.append('response_format', 'verbose_json')
    if (request.language && !request.autoDetect)
    ...
    const segments = rawSegments.map((value) => {
      ...
      const end =
        typeof segment.end === 'number' ? segment.end : Number(segment.end)
      const segmentText =
    ...
    if (
      ...
      throw new AiProviderError(
        'PROVIDER_ERROR',
        'AI provider returned incomplete transcription segments',
        this.providerId
      )
    }
