...
import { ElectronAiCredentialStore, aiCredentialStore } from './ai-credential.service'
import {
  assertPrivacyAllows,
...
import { AiProviderError, AiProviderRegistry, isAiProviderError, type AiFetch, type AiProviderAdapter } from './ai-provider'
...
export interface AiCoreConfig {
  config: Pick<
    AppConfigService,
    'getAiSettings' | 'updateAiProvider' | 'updateAiRoute' | 'setAiPrivacyMode' | 'setAiAllowedCloudDataTypes'
  >
...
export class AiCoreService {
  ...
  public saveProvider(input: AiProviderUpdate): AiSettingsSnapshot {
    ...
    if (!current) throw new AiProviderError('INVALID_CONFIGURATION', 'Unknown AI provider')
    this.validateBaseUrl(input.providerId, input.baseUrl)
    if (typeof input.enabled !== 'boolean') {
      throw new AiProviderError('INVALID_CONFIGURATION', 'AI provider enabled state is invalid', input.providerId)
    }
    if (input.apiKey !== undefined) {
      if (typeof input.apiKey !== 'string' || input.apiKey.length > 4096) {
        throw new AiProviderError('INVALID_CONFIGURATION', 'AI credential is invalid', input.providerId)
      }
  ...
  public setAllowedCloudDataTypes(dataTypes: readonly import('../../../types/ai').AiDataType[]): AiSettingsSnapshot {
  ...
  public async discoverModels(providerId: AiProviderId): Promise<AiModel[]> {
    ...
    if (!config) throw new AiProviderError('INVALID_CONFIGURATION', 'Unknown AI provider')
    assertPrivacyAllows(settings.privacyMode, config, {})
    if (!config.enabled) throw new AiProviderError('PROVIDER_DISABLED', 'AI provider is disabled', providerId)
    const provider = this.getProvider(providerId)
    return provider.discoverModels()
  }

  public async health(providerId: AiProviderId, modelId?: string): Promise<AiProviderHealth> {
    const settings = this.getSettings()
    const config = settings.providers[providerId]
    if (!config) throw new AiProviderError('INVALID_CONFIGURATION', 'Unknown AI provider')
    if (!config.enabled) return { providerId, status: 'DISABLED', ...(modelId ? { modelId } : {}) }
    assertPrivacyAllows(settings.privacyMode, config, {})
    try {
      return await this.getProvider(providerId).health(modelId)
    } catch (error) {
      const normalized = this.normalizeError(error, providerId)
      const status = normalized.code === 'INVALID_CREDENTIALS'
        ? 'INVALID_CREDENTIALS'
  ...
  public transcribe(request: AiTranscriptionRequest): Promise<AiTranscriptionResponse> {
    const classifiedRequest = { ...request, dataTypes: ['transcript' as const] }
    return this.execute('transcription', classifiedRequest, (provider, assignment) => {
      ...
      return provider.transcribe({
        ...
        ...(request.language && !request.autoDetect ? { language: request.language } : {}),
  ...
  private async execute<T>(
    ...
    operation: (provider: AiProviderAdapter, assignment: import('../../../types/ai').AiModelAssignment) => Promise<T>
  ): Promise<T> {
    const settings = this.getSettings()
    const assignments = routeAssignments(settings, task)
    if (assignments.length === 0) {
      throw new AiProviderError('ROUTE_NOT_CONFIGURED', `No AI route configured for ${task}`)
    }
    ...
    for (const assignment of assignments) {
      try {
        ...
        if (!providerConfig) throw new AiProviderError('INVALID_CONFIGURATION', 'Unknown AI provider', assignment.providerId)
        assertPrivacyAllows(settings.privacyMode, providerConfig, {
          ...request,
          allowedDataTypes: settings.allowedCloudDataTypes
        })
        if (!providerConfig.enabled) {
          throw new AiProviderError('PROVIDER_DISABLED', 'AI provider is disabled', assignment.providerId)
        }
        const provider = this.getProvider(assignment.providerId)
        validateAssignmentCapability(task, assignment, provider, request.structured === true)
        const models = await provider.discoverModels()
        const model = models.find((candidate) => candidate.id === assignment.modelId)
        if (!model) throw new AiProviderError('MODEL_MISSING', 'AI model was not found', assignment.providerId)
        if (!model.capabilities.includes(requiredCapabilityForTask(task, request.structured === true))) {
          throw new AiProviderError('CAPABILITY_UNSUPPORTED', 'AI model does not support the requested task', assignment.providerId)
        }
        return await operation(provider, assignment)
      } catch (error) {
        lastError = this.normalizeError(error, assignment.providerId)
        if (!shouldTryAiFallback(lastError) || assignment === assignments[assignments.length - 1]) throw lastError
      }
    }

    throw lastError ?? new AiProviderError('PROVIDER_ERROR', 'AI operation failed')
  }
  ...
  private getProvider(providerId: AiProviderId): AiProviderAdapter {
    ...
    if (!config) throw new AiProviderError('INVALID_CONFIGURATION', 'Unknown AI provider', providerId)
  ...
  private normalizeError(error: unknown, providerId: AiProviderId): AiProviderError {
    if (isAiProviderError(error)) return error
    if (error instanceof Error && error.message === 'Secure credential storage unavailable') {
      return new AiProviderError('INVALID_CREDENTIALS', 'Secure AI credential storage unavailable', providerId)
    }
    return new AiProviderError('PROVIDER_ERROR', 'AI provider operation failed', providerId)
  }
  ...
  private validateBaseUrl(providerId: AiProviderId, baseUrl: string): void {
    if (typeof baseUrl !== 'string' || baseUrl.length === 0 || baseUrl.length > 2048) {
      throw new AiProviderError('INVALID_CONFIGURATION', 'AI provider URL is invalid', providerId)
    }
    try {
      const parsed = new URL(baseUrl)
      if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password || parsed.search || parsed.hash) throw new Error()
      if (providerId === 'openai' && (parsed.protocol !== 'https:' || parsed.hostname !== 'api.openai.com')) throw new Error()
