...
export class RecommendationFeedbackService {
  ...
  public getFeedbackForProfile(db: Database.Database, profileId: string): Map<string, RecommendationFeedbackType> {
  ...
  public recordExposures(db: Database.Database, profileId: string, courseIds: string[]): void {
  ...
  public getRecentExposures(db: Database.Database, profileId: string, sinceTimestamp: number): Set<string> {
    const rows = db.prepare(`
