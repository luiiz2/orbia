...
export function fuseRankedCandidates(
  ...
  lexical.forEach((candidate, rank) =>
    addRank(scores, candidate, 1 / (RECIPROCAL_RANK_OFFSET + rank + 1))
  )
...
export function applyTimestampProximity(
  ...
  return candidates
    .map((candidate) => ({
      ...candidate,
      relevanceScore:
        candidate.relevanceScore + proximityBoost(candidate, moment)
    }))
    .sort(compareCandidates)
}

function addRank(
  scores: Map<string, RankedCandidate>,
