...
export class HybridRetrievalService {
  public constructor(
    private readonly dependencies: HybridRetrievalServiceDependencies
  ) {}

  public async retrieve(
    input: HybridRetrievalRequest
    ...
    const coverage = this.dependencies.repository.getGenerationCoverage(
      generation.id
    )
    const query = input.query.trim()
    if (!query) return { sources: [], coverage, semanticUsed: false }

    const lexical = applyTimestampProximity(
      this.searchLexical(generation.id, query, scope),
      input.moment
    )
  ...
  private searchLexical(
    ...
    try {
      return this.dependencies.repository
        .searchLexical(generationId, query, scope, MAX_VECTOR_CANDIDATES)
        .map((row) => ({
          ...row,
  ...
  private async searchSemantic(
    ...
    try {
      ...
      const queryVector = validQueryVector(
        response,
        generation.providerId,
        generation.modelId,
        generation.dimensions
      ...
      const candidates = this.dependencies.repository
        .listVectorRows(generation.id, scope, MAX_VECTOR_CANDIDATES)
        .filter((row) =>
          matchesGeneration(
            row,
            generation.providerId!,
...
function decodeVector(vector: Buffer, dimensions: number): number[] | null {
  ...
  const values = Array.from(
    new Float32Array(vector.buffer, vector.byteOffset, dimensions)
  )
