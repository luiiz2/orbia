...
export function registerAiIpc(): void {
  ...
  ipcMain.handle(
    ...
    (_event, payload: unknown) =>
      run(() =>
        aiCoreService.setAllowedCloudDataTypes(parseDataTypes(payload) ?? [])
      )
  )
  ...
  ipcMain.handle('ai:health', (_event, payload: unknown) =>
    run(() => {
      ...
      return aiCoreService.health(
        providerId,
        value.modelId as string | undefined
      )
    })
  )
  ...
  ipcMain.handle('ai:clear-storage-category', (_event, payload: unknown) =>
    run(() => {
      ...
      const validCategories: AiStorageCategory[] = [
        'transcripts',
        'semanticIndex',
        'summaries',
        'chatHistory',
      ...
      if (!validCategories.includes(category))
        throw invalid('Invalid storage category')
...
function parseRouteUpdate(payload: unknown): AiRouteUpdate {
  ...
  if (
    ...
    !AI_TASKS.includes(value.task as AiRouteUpdate['task'])
  ) {
    throw invalid('Invalid AI route configuration')
  }
  if (
  ...
  if (assignment.capabilities !== undefined) {
    if (
      ...
      assignment.capabilities.some(
        (capability) => !AI_CAPABILITIES.includes(capability)
      )
    ) {
      throw invalid('Invalid AI route configuration')
    }
...
function parsePrivacyMode(value: unknown) {
  if (
    typeof value !== 'string' ||
    !AI_PRIVACY_MODES.includes(value as (typeof AI_PRIVACY_MODES)[number])
  ) {
...
function parseProviderId(value: unknown) {
  if (
    typeof value !== 'string' ||
    !AI_PROVIDER_IDS.includes(value as (typeof AI_PROVIDER_IDS)[number])
  ) {
...
function parseChatInput(payload: unknown): AiChatInput {
  ...
  if (
    ...
    (typeof value.promptKind !== 'string' ||
      !AI_PROMPT_KINDS.includes(value.promptKind as AiPromptKind))
  ) {
    throw invalid('Invalid AI chat request')
  }
  if (
  ...
  if (
    ...
    typeof value.cloudConsent !== 'boolean'
  )
    throw invalid('Invalid AI chat request')
  return {
    messages,
    ...(value.promptKind !== undefined
      ? { promptKind: value.promptKind as AiPromptKind }
      : {}),
...
function parseEmbeddingRequest(payload: unknown): AiEmbeddingRequest {
  ...
  if (
    (typeof input === 'string' && input.length > 200_000) ||
    (Array.isArray(input) && input.some((item) => item.length > 200_000))
  ) {
    throw invalid('Invalid AI embedding request')
  }
  const dataTypes = parseDataTypes(value.dataTypes)
...
function parseDataTypes(value: unknown) {
  ...
  if (
    !Array.isArray(value) ||
    value.some((item) => !AI_DATA_TYPES.includes(item))
  )
    throw invalid('Invalid AI data classification')
