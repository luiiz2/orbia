...
export class ImportSourceCapabilityRegistry {
  ...
  public consume(
    token: string,
    expectedKind: ImportSourceCapabilityKind
  ): string {
    const capability = this.capabilities.get(token)
    if (!capability)
      throw new Error('Import source capability is invalid or already used.')
...
export class CoverImageSelectionRegistry {
  ...
  public consume(selectedPath: string): string {
    ...
    if (!selection) {
      throw new Error(
        'Cover image must be selected with the native file picker.'
      )
    }
    ...
    if (this.now() >= selection.expiresAt) {
      throw new Error(
        'Cover image selection expired. Select it again with the native file picker.'
      )
    }
 ...
 */
export function normalizeCommitImportSessionPayload(
  payload: unknown
): CommitImportSessionInput | null {
  if (
    !isRecord(payload) ||
 ...
 */
export function normalizeMergePreviewCourseIds(
  payload: unknown
): string[] | null {
  ...
  for (const value of payload.courseIds) {
    ...
    if (!courseId || courseId.length > 200 || /[\\/]/.test(courseId))
      return null
...
export function normalizeImportSourceCapabilityToken(
  payload: unknown
): string | null {
  if (
    !isRecord(payload) ||
...
function normalizeTitleEdits(
  value: unknown
): ImportSessionTitleEdits | undefined | null {
  if (value === undefined) return undefined
  if (!isRecord(value)) return null

  if (value.courseTitle !== undefined && typeof value.courseTitle !== 'string')
    return null
...
function normalizeTitleEditEntries(
  value: unknown
): ImportSessionTitleEdit[] | undefined | null {
  if (value === undefined) return undefined
...
export function registerCoursesIpc(): void {
  ...
  ipcMain.handle('courses:select-cover-image', async () => {
    try {
      const result = await dialog.showOpenDialog({
        ...
        filters: [
          {
            name: 'Imagens (*.jpg, *.png, *.webp, *.jpeg)',
  ...
  ipcMain.handle(
    ...
    async (_event, payload: { courseId: string; coverPath: string }) => {
      try {
        if (
          !payload ||
  ...
  ipcMain.handle(
    ...
    async (_event, payload: { lessonId: string; coverPath: string }) => {
      try {
        if (
          !payload ||
  ...
  ipcMain.handle(
    'courses:extract-thumbnails',
    async (_event, payload?: { courseId?: string }) => {
      try {
        const res = await databaseService.extractMissingVideoThumbnails(
          payload?.courseId
        )
    ...
    async (event, payload: unknown) => {
      try {
        ...
        if (!token) {
          return {
            success: false,
            error: 'A valid import source capability is required.'
          }
        }
        ...
        if (!fs.existsSync(zipPath) || !fs.statSync(zipPath).isFile()) {
          return {
            success: false,
            error: `Zip archive not found at path: ${zipPath}`
          }
        }

        const result = await importSessionService.prepareZipImport({
          zipPath,
          stagingBaseDir: path.join(
            app.getPath('temp'),
            'orbia',
            'import-staging'
          ),
          onProgress: (percent, currentFile) => {
            event.sender.send('courses:extract-progress', {
              percent,
  ...
  ipcMain.handle(
    ...
    async (_event, payload: unknown) => {
      try {
        ...
        const result =
          await importSessionService.prepareFolderImport(folderPath)
        return { success: true, ...result }
      } catch (err: unknown) {
  ...
  ipcMain.handle('courses:extract-zip', async () =>
    ...
    async (_event, payload: unknown) => {
      try {
        ...
        if (!courseIds) {
          return {
            success: false,
            error: 'Select at least two valid courses to preview a merge.'
          }
        }

        return {
          success: true,
          preview: databaseService.getMergePreview(courseIds)
        }
      } catch (err: unknown) {
  ...
  ipcMain.handle('courses:merge-courses', async (_event, payload: unknown) => {
    try {
      ...
      if (!courseIds || courseIds.length < 2) {
        return {
          success: false,
          error: 'Select at least two valid courses to merge.'
        }
      }
  ...
  ipcMain.handle(
    'courses:generate-organization-plan',
    async (_event, courseId: string) => {
      try {
        if (!courseId) return { success: false, error: 'Course ID is required' }
  ...
  ipcMain.handle(
    'courses:apply-organization-plan',
    async (_event, plan: OrganizationPlan) => {
      try {
        if (!plan || !plan.courseId)
          return {
            success: false,
            error: 'Valid organization plan is required'
          }
  ...
  ipcMain.handle(
    'courses:record-import-history',
    async (_event, payload: Omit<ImportHistoryEntry, 'id' | 'importedAt'>) => {
      try {
  ...
  ipcMain.handle('courses:scan-folder', async () =>
    legacyImportUnavailableResult()
  )
      ...
      try {
        ...
        if (payload.deleteFiles) {
          ...
          const inVault =
            vaultRoot &&
            root &&
            path.resolve(root).startsWith(vaultRoot + path.sep)
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      payload: { lessonId: string; deleteFileFromDisk?: boolean }
    ) => {
      try {
        if (
          !payload ||
        ...
        return databaseService.deleteLesson(
          payload.lessonId.trim(),
          Boolean(payload.deleteFileFromDisk)
        )
      } catch (err: unknown) {
  ...
  ipcMain.handle(
    'courses:convert-srt-to-vtt',
    async (_event, payload: { srtPath: string }) => {
      try {
        if (
          !payload ||
          typeof payload.srtPath !== 'string' ||
          !payload.srtPath.trim()
        ) {
        ...
        if (!fs.existsSync(trimmedPath)) {
          return {
            success: false,
            error: `Subtitle file not found at path: ${trimmedPath}`
          }
        }
  ...
  ipcMain.handle(
    ...
    async (_event, payload: { courseId: string }) => {
      try {
        ...
        const plan = reorganizerService.generateReorganizePlan(
          payload.courseId.trim()
        )
        return { success: true, plan }
      } catch (err: unknown) {
    ...
    async (
      ...
      try {
        if (!payload || !payload.groupId || !Array.isArray(payload.mutations)) {
          return {
            success: false,
            error: 'Valid group ID and mutations array are required'
          }
        }
        return reorganizerService.applyReorganizePlan(
          payload.groupId,
          payload.mutations,
          payload.courseId
        )
    ...
    async (_event, payload: { courseId: string; customTitle?: string }) => {
      try {
        if (!payload || !payload.courseId)
          return { success: false, error: 'Course ID is required' }
        databaseService.updateCourseMetadata(payload.courseId, {
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      payload: { moduleId: string; customTitle?: string; displayOrder?: number }
    ) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      payload: { lessonId: string; customTitle?: string; displayOrder?: number }
    ) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, payload: { moduleId: string; direction: 'up' | 'down' }) => {
      try {
        ...
        const success = databaseService.reorderModule(
          payload.moduleId,
          payload.direction
        )
        return { success }
  ...
  ipcMain.handle(
    ...
    async (_event, payload: { lessonId: string; direction: 'up' | 'down' }) => {
      try {
        ...
        const success = databaseService.reorderLesson(
          payload.lessonId,
          payload.direction
        )
        return { success }
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      payload: { moduleId: string; courseId: string; completed?: boolean }
    ) => {
      try {
        if (!payload || !payload.moduleId || !payload.courseId)
