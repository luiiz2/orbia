...
export function registerStudioIpc(): void {
  // --- Appearances ---
  ipcMain.handle(
    'studio:list-appearances',
    async (_event, courseId?: string, includeHidden?: boolean) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return []
      return libraryAppearanceService.listAppearances(
        db,
        courseId,
        includeHidden
      )
  ...
  ipcMain.handle(
    'studio:update-appearance',
    async (_event, id: string, updates: Partial<LibraryAppearance>) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return false
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return libraryAppearanceService.createReference(
        db,
        entityType,
        entityId,
        targetCourseId,
  ...
  ipcMain.handle(
    'studio:delete-appearance',
    async (_event, appearanceId: string) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
  ...
  ipcMain.handle(
    'studio:set-hidden',
    async (_event, appearanceIds: string[], isHidden: boolean) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return false
  ...
  ipcMain.handle(
    ...
    async (_event, courseId: string, title: string, moduleId?: string) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) throw new Error('Database not connected')
  ...
  ipcMain.handle(
    ...
    async (_event, id: string, updates: Partial<LibrarySection>) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return false
  ...
  ipcMain.handle(
    'studio:update-collection',
    async (_event, id: string, updates: Partial<Collection>) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return false
  ...
  ipcMain.handle(
    ...
    async (_event, collectionId: string, appearanceIds: string[]) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return false
  ...
  ipcMain.handle(
    ...
    async (_event, collectionId: string, appearanceIds: string[]) => {
      ...
      if (!db) return false
      return collectionService.removeItemsFromCollection(
        db,
        collectionId,
        appearanceIds
      )
  ...
  ipcMain.handle(
    'studio:delete-custom-field-definition',
  ...
  ipcMain.handle(
    'studio:get-custom-field-values',
  ...
  ipcMain.handle(
    ...
    async (_event, entityId: string, fieldId: string, value: string) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return false
  ...
  ipcMain.handle(
    ...
    async (_event, sourceCourseId: string, targetCourseId: string) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return { success: false, error: 'Database not connected' }
      return structuralOperationsService.courseToModule(
        db,
        sourceCourseId,
        targetCourseId
      )
  ...
  ipcMain.handle(
    'studio:module-to-course',
    async (_event, moduleId: string, newCourseTitle?: string) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return { success: false, error: 'Database not connected' }
      return structuralOperationsService.moduleToCourse(
        db,
        moduleId,
        newCourseTitle
      )
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return structuralOperationsService.moveItems(
        db,
        appearanceIds,
        targetParentId,
        targetCourseId
  ...
  ipcMain.handle(
    ...
    async (_event, appearanceIds: string[], courseTitle: string) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return { success: false, error: 'Database not connected' }
      return structuralOperationsService.createCourseFromSelection(
        db,
        appearanceIds,
        courseTitle
      )
  ...
  // --- Bulk & Spreadsheet ---
  ipcMain.handle(
    'studio:rename-preview',
    async (_event, appearanceIds: string[], options: BulkRenameOptions) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return []
  ...
  ipcMain.handle(
    'studio:rename-apply',
    async (_event, items: { appearanceId: string; newTitle: string }[]) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
      if (!db) return false
  ...
  ipcMain.handle(
    'studio:apply-spreadsheet-draft',
    async (_event, changes: SpreadsheetDraftChange[]) => {
      const db = (
        databaseService as unknown as { db: import('better-sqlite3').Database }
      ).db
  ...
  ipcMain.handle(
    'studio:execute-automation-rule',
  ...
  ipcMain.handle(
    ...
    async (_event, name: string, avatarPath?: string) => {
  ...
  ipcMain.handle(
    ...
    async (_event, id: string, updates: Partial<LocalProfile>) => {
  ...
  ipcMain.handle(
    'studio:save-theme-preset',
    async (
      _event,
      preset: Omit<ThemePreset, 'id' | 'createdAt'> & { id?: string }
    ) => {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return appConfigService.getResolvedTheme(
        profileId,
        vaultPath,
        courseId,
        sectionId
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return appConfigService.saveAppearanceOverride(
        scopeType,
        scopeId,
        overrides,
        presetId
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return appConfigService.resetAppearanceOverride(
        scopeType,
        scopeId,
        category
      )
  ...
  ipcMain.handle(
    'studio:export-theme-package',
    async (_event, presetId: string, targetPath?: string) => {
      return themePackageService.exportTheme(presetId, targetPath)
    }
  )

  ipcMain.handle(
    'studio:import-theme-package',
