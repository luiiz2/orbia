...
export function registerDiscoveryIpc(): void {
  ipcMain.handle('discovery:get-rails', async (_event, profileId?: string): Promise<DiscoveryRail[]> => {
    ...
    const prefs = appConfigService.getProfileDiscoveryPreferences(activeProfileId)
    return discoveryEngineService.getDiscoveryRails(db, activeProfileId, prefs)
  })

  ipcMain.handle('discovery:get-similar-courses', async (_event, courseId: string, limit?: number): Promise<DiscoveryItem[]> => {
    const db = (databaseService as unknown as { db: import('better-sqlite3').Database }).db
    if (!db) return []
    return discoveryEngineService.getSimilarCourses(db, courseId, limit || 6)
  })

  ipcMain.handle('discovery:get-time-recommendations', async (_event, minutes: number, profileId?: string): Promise<TimeBasedRecommendation[]> => {
    const db = (databaseService as unknown as { db: import('better-sqlite3').Database }).db
    if (!db) return []
    return timeMatcherService.getRecommendationsForTimeWindow(db, minutes, profileId)
  })
  ...
  ipcMain.handle('discovery:get-category-discovery', async (): Promise<CategoryDiscoveryData[]> => {
    const db = (databaseService as unknown as { db: import('better-sqlite3').Database }).db
    if (!db) return []
    return discoveryEngineService.getCategoryDiscovery(db)
  })

  ipcMain.handle('discovery:get-insights', async (): Promise<LibraryInsights> => {
    const db = (databaseService as unknown as { db: import('better-sqlite3').Database }).db
    if (!db) {
      return {
        totalCourses: 0,
  ...
  ipcMain.handle('discovery:get-profile-preferences', async (_event, profileId: string): Promise<ProfileDiscoveryPreferences> => {
    return appConfigService.getProfileDiscoveryPreferences(profileId)
  })

  ipcMain.handle('discovery:save-profile-preferences', async (_event, prefs: ProfileDiscoveryPreferences): Promise<boolean> => {
  ...
  ipcMain.handle(
    ...
    async (_event, profileId: string, courseId: string, feedbackType: RecommendationFeedbackType): Promise<boolean> => {
      const db = (databaseService as unknown as { db: import('better-sqlite3').Database }).db
      if (!db) return false
      discoveryEngineService.invalidateCache()
      return recommendationFeedbackService.submitFeedback(db, profileId, courseId, feedbackType)
    }
  )

  ipcMain.handle('discovery:list-relationships', async (_event, courseId?: string): Promise<CourseRelationship[]> => {
    const db = (databaseService as unknown as { db: import('better-sqlite3').Database }).db
    if (!db) return []
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (!db) return null
      discoveryEngineService.invalidateCache()
      return courseRelationshipsService.addRelationship(db, sourceCourseId, targetCourseId, relationshipType)
    }
  )

  ipcMain.handle('discovery:delete-relationship', async (_event, id: string): Promise<boolean> => {
    const db = (databaseService as unknown as { db: import('better-sqlite3').Database }).db
    if (!db) return false
    discoveryEngineService.invalidateCache()
